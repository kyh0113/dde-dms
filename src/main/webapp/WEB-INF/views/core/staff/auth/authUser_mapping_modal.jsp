...
<!-- Modal -->
<div class="modal fade" id="authUserMappingModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="myModalLabel">&lt;<span id="authMenuModal_title"></span>&gt; 권한 사용자 매핑</h4>
      </div>
      <div class="modal-body" style="height:600px;">
     		<form class="form-horizontal" id="authUserForm">
      			<!-- 20191023_khj for csrf -->
				<input type="hidden" name="${_csrf.parameterName}" id="${_csrf.parameterName}" value="${_csrf.token}" />
				<input type="hidden" name="selection_auth_id" id="selection_auth_id" value="" />
				<!-- popup contents -->
				<section>
			...
			<section class="section">
				...
				<div id="authUserCntl-uiGrid"  data-ng-controller="modal_authUserCntl" style="height: 400px;">
      ...
      <div class="modal-footer">
      	<button id="btn_authUserMappingModal_save" type="button" class="btn">저장</button>
        <button id="btn_authUserMappingModal_close" type="button" class="btn" data-dismiss="modal">닫기</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
<script>
	...
	app.controller('modal_authUserCntl', ['$scope','$controller','$log','StudentService', 
		...
		$scope.gridOptions = vm.gridOptions(  				// 그리드 옵션, 부모의 그리드 옵션에 파라미터를 던지면서 변경해서 대입
			{
				...
				columnDefs : [  							//컬럼 세팅
					{ visible:false, displayName: '변경할권한코드',  field: 'auth_id', enableCellEdit : false, allowCellFocus : false, cellClass : "center"},
					{ visible:false, displayName: '접속자ID',  field: 's_emp_code', enableCellEdit : false, allowCellFocus : false, cellClass : "center"},
					{ width:"20%", displayName: '사용자ID', field: 'user_id', enableCellEdit : false, allowCellFocus : false, cellClass : "center"},
		            { displayName: '사용자명',  field: 'user_name', enableCellEdit : false, allowCellFocus : false, cellClass : "center"},
					{ width:"20%", displayName: '현재 권한코드',  field: 'now_auth_id', enableCellEdit : false, allowCellFocus : false, cellClass : "center"},
					{ displayName: '현재 권한명',   field: 'now_auth_name', enableCellEdit : false, allowCellFocus : false, cellClass : "center"},
				]
			}
		);  
...
$(document).ready(function(){
	var userScope = angular.element(document.getElementById("authUserCntl-uiGrid")).scope(); //html id를 통해서 controller scope(this) 가져옴
	...
	var param = {
		listQuery : "Auth.selectUserList", 				//list가져오는 마이바티스 쿼리 아이디
		cntQuery : "Auth.selectUserCnt"					//list cnt 가져오는 마이바티스 쿼리 아이디
	...
	$("#btn_user_search").on("click",function(){
		userScope.reloadGrid({
			...
			auth_id : document.getElementById("selection_auth_id").value
		});
	})
	
	
	$("#btn_authUserMappingModal_save").on("click",function(){
		var selectedRows = userScope.gridApi.selection.getSelectedRows();
		if(isEmpty(selectedRows)){
			swalWarning("권한을 매핑할 사용자를 선택해주십시오.");
			return false;
		}

		swal({
			  icon : "info",
			  text: "<"+$("#authMenuModal_title").text()+"> 권한으로 매핑하시겠습니까?",
			...
			.then(function(result) {
				if (result) {
					...
					$.ajax({
						type : 'POST',
						url : '/core/staff/auth/mappingUserAuth',
						...
						success : function(data) {
							...
							if (data.result > 0) {
								...
								$("#authUserMappingModal").modal("toggle");
							} else {
		...
		$("#btn_authUserMappingModal_close").on("hide.bs.modal", function() {
