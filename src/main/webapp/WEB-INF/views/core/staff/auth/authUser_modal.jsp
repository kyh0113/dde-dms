...
<!-- Modal -->
<div class="modal fade" id="authEmpModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title" id="myModalLabel">&lt;<span id="authEmpModal_title"></span>&gt; 권한 사용자 목록</h4>
      </div>
      <div class="modal-body">
        	<div class="tblType02">
				<!-- 복붙영역(html) 시작, 복붙시 바꿔야할 값들 : id, data-ng-controller  -->
				<div id="authEmpCtrl-uiGrid"  data-ng-controller="authEmpCtrl" style="height: 400px;">
      ...
      <div class="modal-footer">
        <button id="authEmpModal_close" type="button" class="btn" data-dismiss="modal">닫기</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
<script>
	...
	app.controller('authEmpCtrl', ['$scope','$controller','$log','StudentService', 
		...
		$scope.gridOptions = vm.gridOptions(  				// 그리드 옵션, 부모의 그리드 옵션에 파라미터를 던지면서 변경해서 대입
			{
				...
				columnDefs : [  							//컬럼 세팅
					{ displayName: '권한코드', field: 'auth_id',enableCellEdit : false, allowCellFocus : false},
		            { displayName: '권한명', field: 'auth_name',enableCellEdit : false, allowCellFocus : false},
					{ displayName: '사용자ID', field: 'emp_code',enableCellEdit : false, allowCellFocus : false},
		            { displayName: '사용자명', field: 'emp_name',enableCellEdit : false, allowCellFocus : false},  
				]
			}
		);  
...
$(document).ready(function(){
	...
	var param = {
		listQuery : "Auth.authEmpList", 				//list가져오는 마이바티스 쿼리 아이디
		cntQuery : "Auth.authEmpListCnt"						//list cnt 가져오는 마이바티스 쿼리 아이디
	}; 
