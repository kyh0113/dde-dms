...
// 	}
	function fnSendData(i){
		$("input[name=I_SALES_OFF]", opener.document).val($("input[name=VKBUR]:eq("+i+")").val());
		$(".I_SALES_OFF", opener.document).val($("input[name=BEZEI20]:eq("+i+")").val());
		self.close();
	}
</script>
</head>
<body>
	<div id="popup">
		<div class="pop_header">영업소 검색</div>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>
										${list.VKBUR}
										<input type="hidden" name="VKBUR" value="${list.VKBUR}">
									</td>
									<td style="text-align: left;">${list.BEZEI20}<input type="hidden" name="BEZEI20" value="${list.BEZEI20}">
									</td>
								</tr>
							</c:forEach>
						</c:if>
