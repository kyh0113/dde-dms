...
<script type="text/javascript">
	...
	function fnSendData(i){		
		...
		console.log(i);
		if(type=="F"){//등록폼
			$("input[name=LIFNR]", opener.document).val($("input[name=LIFNR]:eq("+i+")").val());		//공급업체 코드
			$("input[name=STCD2]", opener.document).val($("input[name=STCD2]:eq("+i+")").val());		//사업자번호
			$("input[name=NAME1]", opener.document).val($("input[name=NAME1]:eq("+i+")").val());		//상호명
			$("input[name=HKONT]", opener.document).val($("input[name=HKONT]:eq("+i+")").val());		//계정과목
			$("input[name=TXT_50]", opener.document).val($("input[name=TXT_50]:eq("+i+")").val());		//계정과목 설명
			$("input[name=BANKN]", opener.document).val($("input[name=BANKN]:eq("+i+")").val());		//계좌번호
			$("input[name=BVTYP]", opener.document).val($("input[name=BVTYP]:eq("+i+")").val());		//은행유형
			$("input[name=KTOKK]", opener.document).val($("input[name=KTOKK]:eq("+i+")").val());		//계정그룹
			$("input[name=ZTERM]", opener.document).val($("input[name=ZTERM]:eq("+i+")").val());		//지급조건
			$("input[name=TEXT1]", opener.document).val($("input[name=TEXT1]:eq("+i+")").val());		//지급조건
			//opener.parent.fnBANKNinput();
		}else{//개별항목
			opener.scope.gridOptions.data[target].STCD2 = $("input[name=STCD2]:eq("+i+")").val();		//사업자번호
			opener.scope.gridOptions.data[target].NAME1 = $("input[name=NAME1]:eq("+i+")").val();		//거래처명
			opener.scope.gridOptions.data[target].LIFNR = $("input[name=LIFNR]:eq("+i+")").val();		//업체코드
			opener.scope.gridOptions.data[target].NAME1_STATUS = null;									//READONLY 설정
