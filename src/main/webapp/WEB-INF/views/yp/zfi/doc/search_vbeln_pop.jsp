...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		opener.scope.gridOptions.data[target].VBELN = $("input[name=VBELN]:eq("+i+")").val();		//구매오더
		opener.scope.gridOptions.data[target].VTWEG = $("input[name=VTWEG]:eq("+i+")").val();		//유통경로
		opener.scope.gridOptions.data[target].MVGR1 = $("input[name=MVGR1]:eq("+i+")").val();		//자재그룹
...
<body>
	<div id="popup">
		<div class="pop_header">판매오더 검색</div>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="I_VBELN" <c:if test="${req_data.search_type eq 'I_VBELN'}">selected</c:if>>판매문서</option>
						<option value="I_ARKTX" <c:if test="${req_data.search_type eq 'I_ARKTX'}">selected</c:if>>오더내역</option>
					</select>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>
										<fmt:parseNumber value="${list.VBELN}" />
										<input type="hidden" name="VBELN" value="<fmt:parseNumber value="${list.VBELN}"/>">
									</td>
									<td>${list.ARKTX}<input type="hidden" name="ARKTX" value="${list.ARKTX}">
									</td>
									<td>${list.VTWEG}<input type="hidden" name="VTWEG" value="${list.VTWEG}">
									</td>
									<td>${list.SPART}<input type="hidden" name="SPART" value="${list.SPART}">
									</td>
									<td>${list.MVGR1}<input type="hidden" name="MVGR1" value="${list.MVGR1}">
									</td>
								</tr>
							</c:forEach>
						</c:if>
