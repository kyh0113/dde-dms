...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		opener.scope.gridOptions.data[target].MVGR1 = $("input[name=MVGR1]:eq("+i+")").val();		//자재코드
...
<body>
	<div id="popup">
		<div class="pop_header">자재그룹 검색</div>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="I_MVGR1">자재그룹</option>
						<option value="I_BEZEI" selected>내역</option>
					</select>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>${list.BEZEI}<input type="hidden" name="BEZEI" value="${list.BEZEI}">
									</td>
								</tr>
							</c:forEach>
						</c:if>
