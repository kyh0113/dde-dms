...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		opener.scope.gridOptions.data[target].MEINS = $("input[name=MSEHI]:eq("+i+")").val();		//단위
...
<body>
	<div id="popup">
		<div class="pop_header">단위 검색</div>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="I_MSEHI">단위</option>
						<option value="I_MSEHL" selected>단위설명</option>
					</select>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>${list.MSEHI}<input type="hidden" name="MSEHI" value="${list.MSEHI}">
									</td>
									<td>${list.MSEHL}<input type="hidden" name="MSEHL" value="${list.MSEHL}">
									</td>
								</tr>
							</c:forEach>
						</c:if>
