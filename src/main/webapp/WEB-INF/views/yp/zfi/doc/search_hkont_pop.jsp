...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		if(type==""){
			opener.scope.gridOptions.data[target].HKONT = $("input[name=SAKNR]:eq("+i+")").val();
			opener.scope.gridOptions.data[target].ZGLTXT = $("input[name=TXT_50]:eq("+i+")").val();
		}else if(type=="BACT"){
			// 2020-07-23 jamerl - type=="BACT" 패턴 없음
			opener.scope.gridOptions.data[target].BACT = $("input[name=SAKNR]:eq("+i+")").val();
			opener.scope.gridOptions.data[target].BACTTXT = $("input[name=TXT_50]:eq("+i+")").val();
		}
...
<body>
	<div id="popup">
		<div class="pop_header">계정 검색</div>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="I_TXT_50" <c:if test="${req_data.search_type eq 'I_TXT_50'}">selected</c:if>>설명</option>
						<option value="I_HKONT" <c:if test="${req_data.search_type eq 'I_HKONT'}">selected</c:if>>코드</option>
					</select>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>
										<fmt:parseNumber value="${list.SAKNR}" />
										<input type="hidden" name="SAKNR" value="<fmt:parseNumber value="${list.SAKNR}"/>">
									</td>
									<td style="text-align: left;">${list.TXT50}<input type="hidden" name="TXT_50" value="${list.TXT50}">
									</td>
								</tr>
							</c:forEach>
						</c:if>
