...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		opener.scope.gridOptions.data[target].ZCCODE = $("input[name=ZCCODE]:eq("+i+")").val();		//차량코드
		opener.scope.gridOptions.data[target].ZCTEXT = $("input[name=ZCTEXT]:eq("+i+")").val();		//차량코드명
...
<body>
	<div id="popup">
		<div class="pop_header">차량 검색</div>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="I_ZCTEXT" <c:if test="${req_data.search_type eq 'I_ZCTEXT'}">selected</c:if>>차량코드명</option>
						<option value=I_ZCCODE <c:if test="${req_data.search_type eq 'I_ZCCODE'}">selected</c:if>>차량코드</option>
					</select>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>
										<fmt:parseNumber value="${list.ZCCODE}" />
										<input type="hidden" name="ZCCODE" value="<fmt:parseNumber value="${list.ZCCODE}"/>">
									</td>
									<td>${list.ZCTEXT}<input type="hidden" name="ZCTEXT" value="${list.ZCTEXT}">
									</td>
								</tr>
							</c:forEach>
						</c:if>
