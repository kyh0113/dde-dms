...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		$("input[name=TEXT_ZTERM]", opener.document).val($("input[name=TEXT1]:eq("+i+")").val());		//지급조건 설명
...
<body>
	<div id="popup">
		<div class="pop_header">지급조건 검색</div>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="I_ZTERM">코드</option>
						<option value="I_TEXT1" selected>설명</option>
					</select>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>${list.ZTERM}<input type="hidden" name="ZTERM" value="${list.ZTERM}">
									</td>
