...
<body>
	<div id="popup">
		<div class="pop_header">설비오더 검색</div>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="I_AUFNR">설비 오더번호</option>
						<option value="I_KTEXT" selected>내역</option>
					</select>
				...
				<table cellspacing="0" cellpadding="0">
					...
					<c:if test="${fn:length(list) > 0}">
						<c:forEach var="list" items="${list}" varStatus="i">
							<c:choose>
								<c:when test="${empty list.msg}">
									<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
										<td>${(pagination.curPage-1) * pagination.pageSize + (i.index+1)}</td>
										<td>${list.AUART}<input type="hidden" name="AUART" value="${list.AUART}">
										</td>
										<td>
											<fmt:parseNumber value="${list.AUFNR}" />
											<input type="hidden" name="AUFNR" value="<fmt:parseNumber value="${list.AUFNR}"/>">
										</td>
										<td>${list.KTEXT}<input type="hidden" name="KTEXT" value="${list.KTEXT}">
										</td>
										<td>
											<fmt:parseNumber value="${list.KOSTL}" />
											<input type="hidden" name="KOSTL" value="${list.KOSTL}">
										</td>
										<td>${list.LTEXT}<input type="hidden" name="LTEXT" value="${list.LTEXT}">
										</td>
									</tr>
								</c:when>
								<c:otherwise>
									<tr>
										<td align="center" colspan="6">${list.msg}</td>
									</tr>
								</c:otherwise>
							</c:choose>
						</c:forEach>
