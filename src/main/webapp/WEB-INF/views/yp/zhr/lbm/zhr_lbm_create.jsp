...
<body>
			...
			<div class="tbl_box">
				<table cellspacing="0" cellpadding="0">
					...
					<tr>
						<th>신청일자</th>
						<td>
							<input type="text" class="calendar dtp" name="date" id="sdate" size="10" value="${req_data.date}"/>
						</td>
						<th>식사구분</th>
						<td colspan="3">
							<select name="meal_type" >
								<option value="1" <c:if test="${req_data.meal_type eq '1'}">selected</c:if>>조식</option>
								<option value="2" <c:if test="${req_data.meal_type eq '2' || req_data.meal_type == null}">selected</c:if>>중식</option>
								<option value="3" <c:if test="${req_data.meal_type eq '3'}">selected</c:if>>석식</option>
								<option value="4" <c:if test="${req_data.meal_type eq '4'}">selected</c:if>>야식</option>
							</select>
						</td>
					</tr>
					<tr>
						<th>부서</th>
						<td>
							<input type="text" disabled="disabled"  value="${req_data.dept_name}"/><input type="hidden" name="dept_cd" value="${req_data.dept_cd}" >
						</td>
						<th>근무반</th>
						<td>
							<input type="text" disabled="disabled"  value="${req_data.zclst}"/><input type="hidden" name="zclss" value="${req_data.zclss}" >
						</td>
						<th>근무조</th>
						<td>
							<input type="text" disabled="disabled"  value="${req_data.jo_name}"/><input type="hidden" name="schkz" value="${req_data.schkz}" >
						</td>
					</tr>
				</table>
				<div class="btn_wrap">
	...
	<div class="float_wrap">
		<div class="fl">
			<div class="stitle">
				신청현황<span class="require">(조회건수 : <b>{{gridOptions.data.length}}</b> 건, &nbsp; 신청건수 : <b>{{regCnt}}</b> 건, &nbsp; 집계건수 : <b>{{sumCnt}}</b> 건 )</span>
			</div>
		</div>
		<div class="fr">
			<div class="btn_wrap">
				<input type="button" class="btn_g" id="add_btn"  value="추가"/>
				<input type="button" class="btn_g" id="reg_btn"  value="신청등록"/>
				<input type="button" class="btn_g" id="rmv_btn"  value="신청취소"/>
			</div>
		</div>
	</div>
	...
	<script>
		...
		app.controller('shdsCtrl', [ '$scope', '$window', '$controller', '$log', 'StudentService', 'uiGridConstants', function($scope, $window, $controller, $log, StudentService, uiGridConstants) { //$scope(this)는 해당컨트롤러로 진입하기위한 접근지시자라고 보면됨
			...
			var columnDefs1 = [
				...
				{displayName : '상태', field : 'STATUS', visible : false}, 
				{
					displayName : '집계여부',
					field : 'STATUS_NAME',
					visible : true,
		...
		$(document).ready(function() {
			...
			var param = {
					EXEC_RFC  : "N", // RFC 여부
					cntQuery  : "yp_ZHR.retrieveDosilakWCnt", 	
					listQuery : "yp_ZHR.retrieveDosilakList"
			};
