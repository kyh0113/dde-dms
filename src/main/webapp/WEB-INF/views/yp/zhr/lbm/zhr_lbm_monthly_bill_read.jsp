...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    ...
	    $("select[name=ser_year]").append("<option value="+year+">"+year+"년</option>");
	    $("select[name=ser_year]").append("<option value="+last_year+">"+last_year+"년</option>");

	   	if(mon < 10) mon = "0"+mon;
	   	$("select[name=ser_month]").val(mon);
...
<body>
	...
	<form id="frm" name="frm">
		...
		<input type="hidden" name="emp_cd" value="${req_data.emp_cd}" />
		<section>
			<div class="tbl_box">
				<table cellspacing="0" cellpadding="0">
					...
					<tr>
						<th>기준월</th>
						<td>
							<select title="검색연도" name="ser_year"></select>
							-
							<select title="검색달" name="ser_month">
								<option value="01" <c:if test="${req_data.ser_month == '01'}">selected</c:if>>1월</option>
								<option value="02" <c:if test="${req_data.ser_month == '02'}">selected</c:if>>2월</option>
								<option value="03" <c:if test="${req_data.ser_month == '03'}">selected</c:if>>3월</option>
								<option value="04" <c:if test="${req_data.ser_month == '04'}">selected</c:if>>4월</option>
								<option value="05" <c:if test="${req_data.ser_month == '05'}">selected</c:if>>5월</option>
								<option value="06" <c:if test="${req_data.ser_month == '06'}">selected</c:if>>6월</option>
								<option value="07" <c:if test="${req_data.ser_month == '07'}">selected</c:if>>7월</option>
								<option value="08" <c:if test="${req_data.ser_month == '08'}">selected</c:if>>8월</option>
								<option value="09" <c:if test="${req_data.ser_month == '09'}">selected</c:if>>9월</option>
								<option value="10" <c:if test="${req_data.ser_month == '10'}">selected</c:if>>10월</option>
								<option value="11" <c:if test="${req_data.ser_month == '11'}">selected</c:if>>11월</option>
								<option value="12" <c:if test="${req_data.ser_month == '12'}">selected</c:if>>12월</option>
							</select>
						</td>
						<c:if test="${sessionScope.userId eq 'welstory' || sessionScope.userId eq 'jjd714'}">
						<th>성명</th>
						<td><input type="text" name="emp_name" value="${req_data.emp_name}" ></td>
						<th>&nbsp;</th>
						<td>&nbsp;</td>
						</c:if>
					</tr>
				</table>
				<div style="margin-left:110px; margin-top:10px;">*기준월 : 전월 20일 ~ 당월 19일</div>
				<div class="btn_wrap">
	...
	<div class="float_wrap">
		<div class="fl"><div class="stitle">월간 식대 현황</div></div>
	</div>
		...
		app.controller('shdsCtrl', [ '$scope', '$window', '$controller', '$log', 'StudentService', 'uiGridConstants', function($scope, $window, $controller, $log, StudentService, uiGridConstants) { //$scope(this)는 해당컨트롤러로 진입하기위한 접근지시자라고 보면됨
			...
			var columnDefs1 = [
				...
				},
				{
					...
					enableCellEdit : false,
					allowCellFocus : false
					//footerCellTemplate : '<div class="ui-grid-cell-contents center">월간 합계</div>'
				},
				{
					displayName : '조식',
					field : 'BREAKFAST',
					...
					/* aggregationType: function ()
			        {
			            ...
			            for (var i = 0; i < visibleRows.length; i++) {
		                	if(visibleRows[i].entity.BREAKFAST != null) {
		                		sum += 1;
		                	}
			            }
			            return sum;
				...
				},
				{
					...
					/* aggregationType: function ()
			        {
			            ...
			            var visibleRows = $scope.gridApi.core.getVisibleRows($scope.gridApi.grid);
			            for (var i = 0; i < visibleRows.length; i++) {
			            	if(visibleRows[i].entity.LUNCH != null) {
			                	sum += 1;
			            	}
			            }
			            return sum;
				...
				},
				{
					...
					/* aggregationType: function ()
			        {
			            ...
			            var visibleRows = $scope.gridApi.core.getVisibleRows($scope.gridApi.grid);
			            for (var i = 0; i < visibleRows.length; i++) {
			            	if(visibleRows[i].entity.DINNER != null) {
			                	sum += 1;
			            	}
			            }
			            return sum;
				...
				},
				{
					...
					/* aggregationType: function ()
			        {
			            ...
			            var visibleRows = $scope.gridApi.core.getVisibleRows($scope.gridApi.grid);
			            for (var i = 0; i < visibleRows.length; i++) {
			            	if(visibleRows[i].entity.EATNIGHT != null) {
			                	sum += 1;
			            	}
			            }
			            return sum;
...
// 				showGridFooter: false,
			...
			$scope.gridOptions.rowTemplate = 
					'<div ng-class="{ \'grid-emphasis\': grid.appScope.rowGridEmphasis( row ) }" ng-repeat="col in colContainer.renderedColumns track by col.colDef.name" class="ui-grid-cell" ui-grid-cell></div>';
					
			$scope.rowGridEmphasis = function(row) {
				if(row.entity.EMP_NAME == "월간 합계" || row.entity.EMP_NAME == "월간 식대"){
					return true;
				}
				return false;
			}
		...
		$(document).ready(function() {
			...
			$("#excel_btn").click(function(e){
				if(scope.gridOptions.data.length > 0){	//그리드 데이터 존재여부 확인
				    ...
				    var year= $("select[name=ser_year]").val();
				    var mon = $("select[name=ser_month]").val();
				    
				    var sdate = new Date(year, mon-2, 20);
				    sdate = getFormatDate(sdate);
				    var edate = new Date(year, mon-1, 19);
				    edate = getFormatDate(edate);
				    
				    data.push({name : 'sdate', value : sdate});
				    data.push({name : 'edate', value : edate});
				    //----------------------------------------------------
	...
	<script type="text/javascript">
		...
		function fnSearchData(){
			...
			if(fnValidation()){
				$.ajax({
				    url: "/hr/retrievePeoridDosilakList",
				    type: "POST",
				    cache:false,
				    async:true, 
				    data:$("#frm").serialize(),
