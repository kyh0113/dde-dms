...
<body>
	...
	<script>
		...
		$(document).ready(function() {
			...
			var param = {
					EXEC_RFC  : "N", // RFC 여부
					cntQuery  : "yp_ZHR.retrieveDosilakOkDeptListCnt", 	
					listQuery : "yp_ZHR.retrieveDosilakOkDeptList"
			};
			...
			$("#excel_btn").click(function(e){
				if(scope.gridOptions.data.length > 0){	//그리드 데이터 존재여부 확인
					var query_name = "yp_ZHR.retrieveDosilakOkDeptList_excel";	//다운 받을 엑셀쿼리명
					var excel_name = "도시락집계조회현황";							//다운 받을 엑셀파일명
	...
	<script type="text/javascript">
		...
		function fnSearchData(){
			...
			if(fnValidation()){
				$.ajax({
				    url: "/hr/retrieveDosilakOkDeptList",
				    type: "POST",
				    cache:false,
				    async:true, 
				    data:$("#frm").serialize(),
		...
		function fnExelDown(){
			 ...
			 $("#frm").attr("action", "/hr/retrieveDosilakOkDeptList");
			 $("#frm").submit();
