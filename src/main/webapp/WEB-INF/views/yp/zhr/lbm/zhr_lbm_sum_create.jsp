...
<body>
					...
					<tr>
						...
						<td>
							<select name="meal_type" >
								...
								<option value="2" <c:if test="${req_data.meal_type eq '2'}">selected</c:if>>중식</option>
						...
						<td>
							<select name="status" >
								<option value="W" <c:if test="${req_data.status eq 'W'}">selected</c:if>>미집계</option>
								<option value="Y" <c:if test="${req_data.status eq 'Y'}">selected</c:if>>집계완료</option>
							</select>
						</td>
					</tr>
				</table>
	...
	<div class="float_wrap">
		<div class="fl"><div class="stitle">집계현황</div></div>
		<div class="fr">
			<div class="btn_wrap">
				<input type="button" class="btn_g" id="reg_btn"  value="집계"/>
				<input type="button" class="btn_g" id="rmv_btn"  value="집계취소"/>
			</div>
		</div>
	</div>
	...
	<script>
		...
		$(document).ready(function() {
			...
			var param = {
					EXEC_RFC  : "N", // RFC 여부
					cntQuery  : "yp_ZHR.retrieveDosilakOkListCnt", 	
					listQuery : "yp_ZHR.retrieveDosilakOkList"
			};
			...
			$("#excel_btn").click(function(e){
				if(scope.gridOptions.data.length > 0){	//그리드 데이터 존재여부 확인
					var query_name = "yp_ZHR.retrieveDosilakOkList_excel";	//다운 받을 엑셀쿼리명
					var excel_name = "도시락집계등록현황";							//다운 받을 엑셀파일명
	...
	<script type="text/javascript">
		...
		function fnUpdateDosilakOk(){
			...
			if(selectedRows.length > 0){
				...
				$.ajax({
				    url: "/yp/zhr/lbm/updateDosilakOk",
				    type: "POST",
				    cache:false,
				    async:true, 
				    data: data,
				...
				swalWarningCB("집계 등록할 항목을 선택해 주세요.");
			}
		}
		
		function fnupdateDosilakOkCancel(){
			var selectedRows = scope.gridApi.selection.getSelectedRows();	//그리드 선택된 rows data
			...
			if(selectedRows.length > 0){
				...
				$.ajax({
				    url: "/yp/zhr/lbm/updateDosilakOkCancel",
				    type: "POST",
				    cache:false,
				    async:true, 
				    data: data,
				...
				swalWarningCB("집계 취소할 항목을 선택해 주세요.");
			}
		}
		
		function fnExelDown(){
			 $("input[name=excel_flag]").val("1");	
			 $("#frm").attr("action", "/hr/retrieveDosilakOkList");
			 $("#frm").submit();
