...
<body>
	...
	<form id="frm" name="frm" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="emp_code" id="emp_code" value="${req_data.emp_code}" />
		...
		<section>
			<div class="tbl_box">
				...
				<div class="float_wrap">
					<div class="fl">
						<div style="float:left; margin:17px;"></div>
						<div style="float:left;">
							<input type="button" class="btn_g" id="banner_btn" value="소노리조트홈페이지"/>
						</div>
						<div style="float:left; margin:2px;"></div>
						<div style="float:left;">
							<input type="button" class="btn_g" id="excelTemplate_btn" value="소노리조트요금표"/>
						</div>
						<div style="float:left; margin:2px;"></div>
						<div style="float:left;">
							<input type="button" class="btn_g" id="banner2_btn" value="ES리조트홈페이지"/>
						</div>
						<div style="float:left; margin:2px;"></div>
						<div style="float:left;">
							<input type="button" class="btn_g" id="pdfTemplate_btn" value="ES리조트요금표"/>
						</div>
					</div>
					<div class="fr">
						<div class="btn_wrap">
	...
	<div class="float_wrap">
		...
		<div class="fr">
			<div class="btn_wrap">
				<input type="button" class="btn_g" id="resv_btn" value="예약신청"/>
	...
	<script>
		...
		app.controller('shdsCtrl', [ '$scope', '$window', '$controller', '$log', 'StudentService', 'uiGridConstants', function($scope, $window, $controller, $log, StudentService, uiGridConstants) { //$scope(this)는 해당컨트롤러로 진입하기위한 접근지시자라고 보면됨
			...
			// 상세정보 수정 팝업 jj
			$scope.openPop_ENT_modView = function(type, row){
				fnModDetail(type, row);
			};
			
			// 상세정보 팝업 jj
			$scope.openPop_ENT_view = function(type, row){
				fnViewDetail(type, row);
			};
			...
			var columnDefs1 = [
				...
				{displayName : 'seq', field : 'SEQ', visible : false}, 
				{
					...
					enableCellEdit : false,
					allowCellFocus : false
					//cellTemplate : '<div class="ui-grid-cell-contents"><span class="green" ng-if="row.entity.STATUS ==\'W\'">신청</span><span class="blue" ng-if="row.entity.STATUS ==\'Y\'">집계완료</span></div>'
				},
				{
					displayName : '사용기간',
					field : 'USE_PERIOD',
				...
				},
				{
					...
					enableCellEdit : false,
					allowCellFocus : false,
					cellTemplate : '<div class="ui-grid-cell-contents"><span class="ui-grid-cell-contents btn_s"  ng-if="row.entity.STATUS ==\'A\'" ng-click="grid.appScope.openPop_ENT_modView(\'MOD\',row)">대기</span><span class="ui-grid-cell-contents btn_g" ng-if="row.entity.STATUS ==\'S\'" ng-click="grid.appScope.openPop_ENT_view(\'MOD\',row)">승인</span><span class="ui-grid-cell-contents btn_s" style="background-color:#CC3333;border-color:#CC3333;" ng-if="row.entity.STATUS ==\'F\'" ng-click="grid.appScope.openPop_ENT_view(\'MOD\',row)">반려</span></div>'
					
					//cellTemplate : '<div class="ui-grid-cell-contents"><span class="grey" ng-if="row.entity.STATUS ==\'A\'">대기</span><span class="green" ng-if="row.entity.STATUS ==\'S\'">승인</span><span class="red" ng-if="row.entity.STATUS ==\'F\'">반려</span></div>'
				}
			  ];
		...
		$(document).ready(function() {
			...
			$("#banner_btn").on("click", function(){
				window.open("https://www.sonohotelsresorts.com","bannerPop","");
			});

			// 배너
			$("#banner2_btn").on("click", function(){
				window.open("https://www.clubes.co.kr","bannerPop","");
			});
		...
		function fnEntDetail(type, row){
			...
			window.open("","REZ_POP","scrollbars=yes,width=900,height=550");
			document.ent_frm.target = "REZ_POP";
		...
		//jj pop View
		function fnViewDetail(type, row){
			//form 초기화
			 $("form[name='ent_frm']").each(function() {
			       this.reset();
			      //$("input[type=hidden]").val(''); //reset만으로 hidden type은 리셋이 안되기 때문에 써줌
			 });
			 console.log("---> " + row.entity.SEQ);
			if(type=="VIEW"){
			 	$("#ent_frm input[name=TYPE]").val(type);
			 	$("#ent_frm input[name=SEQ]").val(row.entity.SEQ);
	 			$("#ent_frm input[name=ENT_CODE]").val(row.ENT_CODE);
			...
			console.log("---> " + $("#ent_frm input[name=SEQ]").val());
			
			window.open("","REZ_POP","scrollbars=yes,width=900,height=550");
			document.ent_frm.target = "REZ_POP";
			//document.ent_frm.action = "/yp/popup/zwc/ent/actlDetail";
			document.ent_frm.action = "/yp/popup/zhr/rez/zhrRezView";
			
			$("#ent_frm").submit();
		}

		//jj pop modify
		function fnModDetail(type, row){
			//form 초기화
			$("form[name='ent_frm']").each(function(){
			      this.reset();
			      //$("input[type=hidden]").val(''); //reset만으로 hidden type은 리셋이 안되기 때문에 써줌
			...
			document.ent_frm.target = "REZ_POP";
			//document.ent_frm.action = "/yp/popup/zwc/ent/actlDetail";
			document.ent_frm.action = "/yp/popup/zhr/rez/zhrRezModView";
		...
		$("#excelTemplate_btn").on("click",function(){
			...
			//input.value = "2023년소노리조트객실요금표.xlsx";
			input.value = "2025년소노리조트객실요금표.xlsx";
			input.type = "hidden";
		...
		/****************************************PDF 템플릿 다운로드 시작****************************************/
		$("#pdfTemplate_btn").on("click",function(){

			var form = document.createElement("form");
			var input = document.createElement("input");
			input.name = "file_name";
			input.value = "ES리조트 객실요금표.pdf";
			input.type = "hidden";
			form.appendChild(input);
