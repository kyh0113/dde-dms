...
<body>
		...
		<div class="tbl_box">
			<table cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<th>입장기간</th>
					<td>
						<input type="text" id="A_START_DATE_START" class="default_hide calendar search_dtp_d" value="${today}" readonly="readonly"/>&nbsp;~&nbsp;
						<input type="text" id="A_START_DATE_END" class="default_hide calendar search_dtp_d" value="${today}" readonly="readonly"/>
					</td>
				</tr>
			</table>
			<div class="btn_wrap">
	...
	$(document).ready(function() {
		...
		$("#search_btn").on("click", function() {
			...
			// 작업장 조회조건 IN 사용
			var vaplz_arr = $("#VAPLZ").val().trim().split(",");
			var vaplz_in = "";
			$.each(vaplz_arr, function(i,d){
				console.log(i,d);
				if( i === 0 ){
					vaplz_in += "'" + d + "'";
				}else{
			...
			$.ajax({
				url : "/yp/zcs/ipt/select_zcs_ipt_process_list",
				type : "post",
				cache : false,
				async : true,
				data : {
					VAPLZ : vaplz_in === "''" ? "" : vaplz_in,
					KOSTL : $("#KOSTL").val().trim(),
					A_START_DATE_START : $("#A_START_DATE_START").val().replace(/\//gi, "").trim(),
					A_START_DATE_END : $("#A_START_DATE_END").val().replace(/\//gi, "").trim()
				},
				dataType : "json",
				success : function(data) {
					...
					$.each(result, function(i, d){
						...
						data2 +=
						'<tr class="data_rows">' + 
							'<td class="'+ (flag ? 'add_rowspan row_' + i: 'remove') +'" title="' + (d.VAPLZ === null ? "" : d.VAPLZ) + '">' + (d.VATXT === null ? "" : d.VATXT) + '</td>' + 
		...
		$("#excel_btn").on("click", function() {
			...
			xlsForm.name = "sndFrm";
			xlsForm.method = "post";
			xlsForm.action = "/yp/xls/zcs/ipt/select_zcs_ipt_process_list";
