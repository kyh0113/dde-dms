...
<body>
			...
			<div class="tbl_box">
				<table class="contract_standard_table" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<th></th><td></td>
						<th>작업 다운로드</th>
						<td><input type="button" class="btn btn_make" id="excelDownload_btn" value="ExcelDown"/></td>
					</tr>
				</table>
				<div class="btn_wrap">
	...
	<!-- 엑셀 데이터 다운로드 전용 폼(정해진 엑셀템플릿에 쿼리데이터만 끼워주는 방식) -->
	<form id="excelDownForm" method="post" action="/core/excel/excelDown">
		<input type="hidden" id="listQuery" name="listQuery" value="yp_zcs_ipt.select_construction_monthly_rpt2_excel">
		<input type="hidden" id="filename" name="filename" value="mon_create_sample2">
	...
	app.controller('shdsCtrl', [ '$scope', '$window', '$controller', '$log', 'StudentService', 'uiGridConstants', function($scope, $window, $controller, $log, StudentService, uiGridConstants) { //$scope(this)는 해당컨트롤러로 진입하기위한 접근지시자라고 보면됨
		...
		var columnDefs1 = [
			...
			},
			{
				...
				cellTemplate : '<div class="ui-grid-cell-contents relatvie_box height_full"><input class="absolute_center width_90_percent" type="text" ng-model="row.entity.QTY"/></div>',
// 				footerCellTemplate: '<div class="ui-grid-cell-contents center">{{col.getAggregationValue()}}</div>'
	...
	$(document).ready(function(){
		...
		$("#excelTemplate_btn").on("click",function(){
			...
			input.value = "mon_create_sample2.xlsx";
			input.type = "hidden";
		...
		/****************************************엑셀 데이터 다운로드 시작****************************************/
		$("#excelDownload_btn").on("click", function() {
			var param2 = $("input[name=CONTRACT_CODE]").val();
			
			if(param2 == ""){
				swalWarning("계약코드를 입력해주세요."); 
				return false;
			}
			...
			xlsForm.name = "sndFrm";
			xlsForm.method = "post";
			xlsForm.action = "/yp/xls/zcs/ipt/select_construction_monthly_rpt2_excel";
		...
		$("#remove").on("click", function() {
			...
			//IS_NEW가 Y가 아닐때 
				...
				if (confirm("삭제하겠습니까?")) {
					...
					$.ajax({
						url : "/yp/zcs/ipt/month_opt_delete",
						type : "POST",
						cache : false,
						async : true,
						dataType : "json",
			...
			swal({
				...
				.then(function(result){
				  if(result){
						...
						$.ajax({
							url : "/yp/zcs/ipt/zcs_ipt_mon_opt_create_save",
							type : "POST",
							cache : false,
							async : true,
							dataType : "json",
...
//					data[i]["value"] = data[i]["value"].replace(/ /gi, "");// 모든 공백을 제거
		...
		$.ajax({
			url : "/yp/zcs/ipt/month_opt_excelUpload_check",
			type : "POST",
			cache : false,
			async : false, //동기화
			dataType : "json",
			data : data,
			success : function(result) {
				isExistMonthlyRpt2 = result.result;
			},
			beforeSend : function(xhr) {
				// 2019-10-23 khj - for csrf
				xhr.setRequestHeader(header, token);
