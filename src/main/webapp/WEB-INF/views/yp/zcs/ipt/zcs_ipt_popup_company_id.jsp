...
<script type="text/javascript">
	...
	function fnSendData(i){
		$("#COMPANY_ID", opener.document).val($("input[name=COMPANY_ID]:eq("+i+")").val());		//보고서코드 
		self.close();
	}
</script>
</head>
<body>
	<div id="popup">
		<div class="pop_header">거래처 검색</div>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>${list.COMPANY_ID}<input type="hidden" name="COMPANY_ID" value="${list.COMPANY_ID}"></td>
								</tr>
							</c:forEach>
						</c:if>
						<c:if test="${fn:length(list) == 0}">
							<tr>
								<td align="center" colspan="2">조회된 내역이 없습니다</td>
							</tr>
						</c:if>
					</table>
				</div>
