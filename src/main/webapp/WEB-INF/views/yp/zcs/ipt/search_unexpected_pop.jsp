...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		$("span.auftext_view:eq(" + target + ")", opener.document).text("(" + $("input[name=AUFNR]:eq("+i+")").val() + ") " + $("input[name=AUFTEXT]:eq("+i+")").val());
		$("input.datas:eq(" + target + ")", opener.document).attr("AUFNR", $("input[name=AUFNR]:eq("+i+")").val().trim());
		$("input.datas:eq(" + target + ")", opener.document).attr("AUFTEXT", $("input[name=AUFTEXT]:eq("+i+")").val().trim());
		$("input.datas:eq(" + target + ")", opener.document).attr("VAPLZ", $("input[name=VAPLZ]:eq("+i+")").val().trim());
		$("input.datas:eq(" + target + ")", opener.document).attr("VATXT", $("input[name=VATXT]:eq("+i+")").val().trim());
		$("input.datas:eq(" + target + ")", opener.document).attr("KOSTL", $("input[name=KOSTL]:eq("+i+")").val().trim());
		$("input.datas:eq(" + target + ")", opener.document).attr("KTEXT", $("input[name=KTEXT]:eq("+i+")").val().trim());
...
<body>
	...
	<div id="popup">
		<div class="pop_header">돌발 작업/오더 목록</div>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="1">오더번호</option>
						<option value="2" selected="selected">작업장명</option>
					</select>
