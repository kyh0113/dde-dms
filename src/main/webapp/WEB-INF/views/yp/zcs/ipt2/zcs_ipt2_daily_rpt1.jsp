...
<!-- 							<a href="#" onclick="fnSearchPopup('1');"><img src="/resources/yp/images/ic_search.png"></a> -->
	...
	$(document).ready(function(){
		...
		$("#search_btn").on("click", function() {
			...
			$.ajax({
				url : "/yp/zcs/ipt2/select_zcs_ipt2_daily_rpt1",
				type : "POST",
				cache : false,
				async : true,
				dataType : "json",
...
// 						innerHtml += '	<td class="right vertical-center"	style="width: 100px;"><input type="number" value="' + d.MANHOUR_CORRECTION +	'" data-rn="' + i + '" class="data-col-2 data" step="0.01" readonly="readonly" onchange="javascript: reflectManHour(this);" style="width: 100%; text-align: right;"/></td>';
						...
						innerHtml += '	<td class="right vertical-center"	style="width: 100px;"><input type="number" value="' + d.MANHOUR_CORRECTION +	'" data-rn="' + i + '" class="data-col-2 data" step="0.01" readonly="readonly" style="width: 100%; text-align: right;"/></td>';
			...
			swal({
				...
				if(result){
					...
					if( delete_datas.length > 0 ){
						...
						var header = $("meta[name='_csrf_header']").attr("content");
						$.ajax({
							url : "/yp/zcs/ipt2/delete_zcs_ipt2_daily_rpt1",
							type : "POST",
							cache : false,
							async : false,
							dataType : "json",
			...
			if(fnValidation()){
				swal({
					...
					if(result){
						...
						var header = $("meta[name='_csrf_header']").attr("content");
						$.ajax({
							url : "/yp/zcs/ipt2/merge_zcs_ipt2_daily_rpt1",
							type : "POST",
							cache : false,
							async : false,
							dataType : "json",
			...
			if(fnValidation_confirm( ( exec === "CF" ? "확정" : "확정취소" ) )){
				...
				swal({
					...
					if(result){
						...
						var header = $("meta[name='_csrf_header']").attr("content");
						$.ajax({
							url : ( exec === "CF" ? "/yp/zcs/ipt2/update_confirm_zcs_ipt2_daily_rpt1" : "/yp/zcs/ipt2/update_cancel_zcs_ipt2_daily_rpt1" ),
							type : "POST",
							cache : false,
							async : false,
							dataType : "json",
...
// 			console.log(i, d);
		...
		var header = $("meta[name='_csrf_header']").attr("content");
		$.ajax({
			url : "/yp/zcs/ipt2/pre_select_zcs_ipt2_daily_rpt1",
			type : "POST",
			cache : false,
			async : false,
			dataType : "json",
