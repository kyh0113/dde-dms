...
<body>
			...
			<c:choose>
				<c:when test="${P_RESPONSE_VIEW eq 'Y'}">
					<tr>
						...
						<th>계약</th>
						<td>
							<input type="text" id="CONTRACT_CODE" name="CONTRACT_CODE" size="10" value="${P_CONTRACT_CODE}" readonly="readonly" />
					...
					<tr>
						<th>계약 인원</th>
						<td>
							<input type="text" id="CONTRACT_PEOPLE_CNT" name="CONTRACT_PEOPLE_CNT" value="${P_CONTRACT_PEOPLE_CNT}" disabled="disabled"/>
						</td>
						<th>일일 필수 출근 인원</th>
						<td colspan="3">
							<input type="text" id="DAILY_REQ_PEOPLE_CNT" name="DAILY_REQ_PEOPLE_CNT" value="${P_DAILY_REQ_PEOPLE_CNT}" disabled="disabled"/>
						</td>
					</tr>
				</c:when>
				<c:otherwise>
					...
					<tr>
						<th>계약 인원</th>
						<td>
							<input type="text" id="CONTRACT_PEOPLE_CNT" name="CONTRACT_PEOPLE_CNT" disabled="disabled"/>
						</td>
						<th>일일 필수 출근 인원</th>
						<td colspan="3">
							<input type="text" id="DAILY_REQ_PEOPLE_CNT" name="DAILY_REQ_PEOPLE_CNT" disabled="disabled"/>
						</td>
					</tr>
				</c:otherwise>
			</c:choose>
	...
	$(document).ready(function(){
		...
		$("#search_btn").on("click", function() {
			...
			$.ajax({
				url : "/yp/zcs/ipt2/select_zcs_ipt2_daily_aprv1",
				type : "POST",
				cache : false,
				async : true,
				dataType : "json",
...
// 						innerHtml += '	<td class="right vertical-center"><div style="padding: 4px 3px;">' + ( manhour_sum - manhour_cnt ) + '</div></td>';
		...
		$("#btn_lock, #btn_release").on("click", function() {
			...
			var exec_url = ( exec === "L" ? "/yp/zcs/ipt2/lock_zcs_ipt2_daily_aprv1" : "/yp/zcs/ipt2/release_zcs_ipt2_daily_aprv1" );
			...
			if(fnValidation_confirm( ( exec === "CF" ? "승인" : "승인취소" ) )){
				...
				swal({
					...
					if(result){
						...
						var header = $("meta[name='_csrf_header']").attr("content");
						$.ajax({
							url : ( exec === "CF" ? "/yp/zcs/ipt2/update_aprv_zcs_ipt2_daily_aprv1" : "/yp/zcs/ipt2/update_reject_zcs_ipt2_daily_aprv1" ),
							type : "POST",
							cache : false,
							async : false,
							dataType : "json",
