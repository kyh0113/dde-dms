		...
		$.ajax({
			...
			success : function(result) {
					...
					for(var key in work_status_list[i]){
						...
						if(!isNaN(key)){
							...
							//비어있지 않을 경우에만 cost_sum, total_sum 합계 구하기
								...
								cost_sum  += scope.formatter_decimal(obj[key]);
								total_sum += scope.formatter_decimal(obj[key]);
							}
							innerHtml += '<td class="center vertical-center" >'+obj[key]+'</td>';
		    			}
					}
					innerHtml += '<td class="center vertical-center" >'+scope.formatter_decimal(cost_sum)+'</td>';
					innerHtml += '</tr>';
					$('#work_status_table').append(innerHtml);
				}
				//----------------------------------------------------
				...
				for(var key in sum_by_date_map){
	    			//날짜별 합계 만들어주기
					innerHtml += '	<td class="center vertical-center" >'+scope.formatter_decimal(sum_by_date_map[key])+'</td>';
				}
				//총합계
				innerHtml += '	<td class="center vertical-center" >'+scope.formatter_decimal(total_sum)+'</td>';
				innerHtml += '</tr>';
				$('#work_status_table').append(innerHtml);
				//----------------------------------------------------
			},
