...
<body>
			...
			<div class="tbl_box">
				<table>
					...
					<tr>
						<th>요청부서</th>
						<td>
							<input type="text" name="req_dept_name" value="${req_data.req_dept}">
						</td>
						<th>요청자</th>
						<td>
							<input type="text" name="req_user_name" value="${req_user_id}">
						</td>
						<th>비품유형</th>
						<td>
			        ...
			        <tr>
						...
						<td>
							 ...
							 * Z : 미상신
							-->
							<input id="edoc_status_all" type="radio" name="edoc_status" value="" checked="checked">
							<label for="edoc_status_all">전체</label>
							<input id="edoc_status_unkown" type="radio" name="edoc_status" value="Z">
							<label for="edoc_status_unkown">미상신</label>
							<input id="edoc_status_in_progress" type="radio" name="edoc_status" value="0">
							<label for="edoc_status_in_progress">결재중</label>
							<input id="edoc_status_complete" type="radio" name="edoc_status" value="F">
							<label for="edoc_status_complete">결재완료</label>
							<input id="edoc_status_reject" type="radio" name="edoc_status" value="5">
							<label for="edoc_status_reject">반려</label>
						</td>
			        </tr>
				</table>
				<div class="btn_wrap">		
	...
	<div class="float_wrap" style="margin-bottom:5px;">
		<div class="fr">
			<div class="btn_wrap">
				<input type="button" class="btn_g" id="detail_btn"  value="상세보기"/>
<!-- 				<input type="button" class="btn_g" id="complete_btn"  value="발주요청"/> -->
			</div>
		</div>
	</div>
	<section class="section">
		<!-- 복붙영역(html) 시작, 복붙시 바꿔야할 값들 : id, data-ng-controller  -->
		<div id="fixture-uiGrid"  data-ng-controller="fixtureReqListCtrl" style="height: 600px;">
	...
	app.controller('fixtureReqListCtrl', ['$scope','$controller','$log','StudentService', 'uiGridConstants', 
	function ($scope, $controller, $log, StudentService, uiGridConstants) {		//$scope(this)는 해당컨트롤러로 진입하기위한 접근지시자라고 보면됨
		...
		$scope.rowClick = function(data) {
			...
			//비품요청상세 팝업
			fixtureReqDtlPop(rows);
		}
		
		/**
		 * 결재상신
		 */
		$scope.fnApproval = function(row) {
			/**
			 * 결재 상신 가능 여부 체크
			 */
			is_available_edoc_approval(row)
				.then(function(oResult){
					if(oResult.result){
						/**
						 * window.open(URL, name, specs, replace)
						 * 비품요청서 양식 번호 : EF165657639809073
						 * Parameter : codes
						 * parameter 여러개일 경우 ';'로 합쳐서 보내주자.
						 */
						//개발
						var codes = row.entity.FIXTURE_REQ_CODE,
							url = "http://gwdev.ypzinc.co.kr/ekp/eapp/app.do?cmd=appWrite&eappDoc.formId=EF165663577865476&CODES="+codes;
							
						window.open(url, "전자결재", "scrollbars=auto,width=1000,height=900");
					}else{
						swalWarningCB("이미 결재 상신된 항목입니다.");
					}
				})
		...
		$scope.gridOptions = vm.gridOptions( // 그리드 옵션, 부모의 그리드 옵션에 파라미터를 던지면서 변경해서 대입
		{
			...
			paginationPageSizes : [ 100, 200, 300, 400, 500, 1000 ], //한번에 보여질 로우수 셀렉트리스트	
			...
			columnDefs : [ //컬럼 세팅
			{
				...
				displayName : 'FIXTURE_REQ_CODE',
				field : 'FIXTURE_REQ_CODE',
				/* width : '100', */
				visible : false,
				cellClass : "center",
				pinnedLeft : true,
				...
				enableCellEdit : false,
				allowCellFocus : false,
				cellTemplate : '<div class="ui-grid-cell-contents" style="cursor: pointer; text-decoration: underline;" ng-click="grid.appScope.rowClick(row)">{{row.entity.FIXTURE_TYPE_LIST}}</div>'
			}, {
				displayName : '전자결재',
				field : 'EDOC_STATUS',
				/* width : '100', */
				 ...
				 */
				cellTemplate : 
					 '<div ng-if="!grid.appScope.isApproval(row.entity.EDOC_STATUS)" class="ui-grid-cell-contents btn_g"  style="cursor: pointer;" ng-click="grid.appScope.fnApproval(row)">결재상신</div>'
				...
				allowCellFocus : false,
				cellTemplate : 
					 '<div ng-if="row.entity.REQ_STATUS == \'U\'" class="ui-grid-cell-contents">미완료</div>'
					+'<div ng-if="row.entity.REQ_STATUS == \'I\'" class="ui-grid-cell-contents">발주중</div>'
					+'<div ng-if="row.entity.REQ_STATUS == \'E\'" class="ui-grid-cell-contents">불출완료</div>'
			}, {
...
$(document).ready(function(){
	...
	var param = {
		listQuery : "yp_fixture.grid_fixture_req_list", 				//list가져오는 마이바티스 쿼리 아이디
		cntQuery : "yp_fixture.grid_fixtrue_req_list_cnt"						//list cnt 가져오는 마이바티스 쿼리 아이디
	}; 
	...
	$("#search_btn").on("click",function(){		
		 ...
		 */
		scope.reloadGrid({
			req_dept_name : $("input[name=req_dept_name]").val(),
			req_user_name : $("input[name=req_user_name]").val(),
			...
			edoc_status : $("input[name=edoc_status]:checked").val()
		});
	});
...
/**
 * [결재 상신] 버튼 가능여부 체크
 * 전자결재 상태가 empty상태 일때만 상신가능
 */
function is_available_edoc_approval(row){
	return new Promise(function(resolve, reject) {
		var token = $("meta[name='_csrf']").attr("content");
		var header = $("meta[name='_csrf_header']").attr("content");
		$.ajax({
			url : "/yp/fixture/isAvailableEdocApproval",
			type : "POST",
			cache : false,
			async : true,
			dataType : "json",
			data : {
				row : JSON.stringify(row.entity)
			},
...
function fixtureReqDtlPop(row){
	 ...
	 * parameter 여러개일 경우 ';'로 합쳐서 보내주자.
	 */
	window.open("", "비품요청상세", "width=1200, height=800");
	 
	/**
	 * fnHrefPopup(url, target, pr)
	 */
	fnHrefPopup("/yp/popup/fixture/fixtureReqDtlPop", "비품요청상세", {
