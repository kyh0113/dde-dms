...
<%
	...
	SimpleDateFormat date_1 = new SimpleDateFormat("yyyy/MM");
	String toDay = date.format(today);
	String toDay_1 = date_1.format(today)+"/01";
%>
<head>
...
<script type="text/javascript">
	...
	//공장선택
	function fn_change_ser_factory(num) {

		if (num == "2") {

			//$("select[name=ser_line]").prepend(new Option("전체", ""));
		}
...
<body>
			...
			<div class="tbl_box">
				<table cellspacing="0" cellpadding="0">
					<tr>
						<th>공장</th>
						<td>
							<select name="ser_factory" id="ser_factory" onchange="fn_change_ser_factory(this.value);">
						...
						<th>계열</th>
						<td>
							<select name="ser_line" id="ser_line" onchange="fn_change_ser_line(this.value)">
								<option value="ALL" selected>전체</option>
						...
						<th>조회구분</th>
						<td>
							<select name="ser_gbdt" id="ser_gbdt">
								<option value="A" selected>청소일자</option>
								<option value="B">기준일자</option>
							</select>
						</td>
						<th>조회일자</th>
						<td>
							<input class="calendar dtp" type="text" name="sdate" id="sdate" value="<c:choose><c:when test="${empty req_data.sdate}"><%=toDay_1%></c:when><c:otherwise>${req_data.sdate}</c:otherwise></c:choose>">
							~
							<input class="calendar dtp" type="text" name="edate" id="edate" value="<c:choose><c:when test="${empty req_data.edate}"><%=toDay%></c:when><c:otherwise>${req_data.edate}</c:otherwise></c:choose>">
						</td>
					</tr>
				</table>
				<div class="btn_wrap">
	...
	<div class="float_wrap">
		<div class="fl">
			<div class="stitle">청소실적 현황</div>
		</div>
		<div class="fr">
			<div class="btn_wrap">
				<input type=button class="btn_g" id="fnFunc_01" value="청소실적월별현황">
			</div>
		</div>
	</div>
	...
	<script>
			...
			var columnDefs1 = [
				{
					...
					cellTemplate : '<div class="ui-grid-cell-contents">{{grid.appScope.formatter_date(row.entity.STD_DT)}}</div>'
				}, {
					displayName : '공장',
					field : 'FACTORY_NM',
					//width : '100',
					...
					enableCellEdit : false,
					allowCellFocus : false,
					cellTemplate : '<div class="ui-grid-cell-contents">{{grid.appScope.formatter_date(row.entity.CLEAN_DT)}}</div>'
				}, {
					displayName : '이전청소일',
					field : 'BF_DT',
					//width : '100',
				...
				//{
					...
					//enableCellEdit : false,
					//allowCellFocus : false,
					//cellTemplate : '<div class="ui-grid-cell-contents">{{grid.appScope.formatter_date(row.entity.OVER_DT)}}</div>'
				//},
				{
					displayName : '청소경과일',
					field : 'CALC_CLEAN',
		...
		$(document).ready(function() {
			...
			var param = {
				EXEC_RFC  : "N", // RFC 여부
				listQuery : "yp_zpp.select_zpp_cre_list", //list가져오는 마이바티스 쿼리 아이디
				cntQuery : "yp_zpp.select_zpp_cre_list_cnt" //list cnt 가져오는 마이바티스 쿼리 아이디
			};
			...
			$("#excel_btn").click(function(e){
				if(scope.gridOptions.data.length > 0){ //그리드 데이터 존재여부 확인
					var query_name = "yp_zpp.retrieveCreOkList_excel"; //다운 받을 엑셀쿼리명
					var excel_name = "청소실적등록현황"; //다운 받을 엑셀파일명
		...
		function popup_Query_CreList() {
			...
			//var popupX = (document.body.offsetWidth - 500) / 2;
			//var popupY = (document.body.offsetHeight - 100) / 2;

			window.open("","popup_Cre_list","width=1250,height=600,left=0,top=0" + ",scrollbars=yes");

			form.method = "post";
			form.target = "popup_Cre_list"
			form.action = "/yp/popup/zpp/cre/zpp_cre_popup1";
