...
<sec:csrfMetaTags />
<title>
	<c:if test="${req_data.type eq 'Z'}">담당부서 검색</c:if>
...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		if(type=="Z"){//담당부서
			...
			opener.scope.gridOptions.data[target].DEPT_CODE = $("input[name=KOST1]:eq("+i+")").val();	//담당부서
			opener.scope.gridOptions.data[target].DEPT_NAME = $("input[name=VERAK]:eq("+i+")").val();	//담당부서 설명
			opener.scope.gridApi.grid.refresh();
		}else if(type=="C"){
			$("input[name=I_GUBUN]").val("C");
			opener.scope.gridOptions.data[target].COST_CODE = $("input[name=KOST1]:eq("+i+")").val();	//계정과목
			opener.scope.gridOptions.data[target].COST_NAME = $("input[name=VERAK]:eq("+i+")").val();	//계정과목 설명
			opener.scope.gridApi.grid.refresh();
		} else if(type=="CT"){
			<%-- 2022-01-04 jamerl - 돌발작업 로직 추가 --%>
			$(".data-base .rows-checkbox.rows-no-" + target, opener.document).attr("COST_CODE", $("input[name=KOST1]:eq("+i+")").val().trim());
			$(".data-base .rows-checkbox.rows-no-" + target, opener.document).attr("COST_NAME", $("input[name=VERAK]:eq("+i+")").val().trim());
			$(".data-base .cost_code.rows-no-" + target, opener.document).val( $("input[name=KOST1]:eq("+i+")").val() );
			$(".data-base .cost_name.rows-no-" + target, opener.document).val( $("input[name=VERAK]:eq("+i+")").val() );
		}
...
<body>
	<div id="popup">
		<div class="pop_header">
			...
			<c:if test="${req_data.type eq 'C'}">코스트센터 검색</c:if>
			<%-- 2022-01-04 jamerl - 돌발작업 로직 추가 --%>
			<c:if test="${req_data.type eq 'CT'}">코스트센터 검색</c:if>
		</div>
