...
<%-- 									<input type="hidden" id="DEPT_CODE" value="${sessionScope.userDeptCd}"> --%>
	...
	<div class="float_wrap" style="margin-bottom: 2px;">
		<div class="fl">
			<div class="btn_wrap">
				※【계약명】을 클릭하면 해당일의 상세정보가 두 번째 그리드에 조회됩니다. ※【근무】가 『휴일』상태면 저장시 투입공수, 실공수, 작업량이 『0』으로 저장됩니다. ※【일보확정】데이터 존재시 【계약명】항목이 <span style="background-color: darkorange;">오렌지색</span>으로 표시됩니다.
			</div>
		</div>
		<div class="fr">
			<div class="btn_wrap">
	...
	<div class="float_wrap" style="margin-bottom: 2px; margin-top: -20px;">
		<div class="fl">
			<div class="btn_wrap">
				※ 【일보확정 데이터】가 아래 출력됩니다. ※ 【일보확정 데이터】는 태그데이터가【공수】, 추가데이터가【보정공수】에 출력됩니다.
			</div>
		</div>
		<div class="fr">
			<div class="btn_wrap">
			...
			var columnDefs = [
				{
					...
					cellTemplate : 
						...
						'<select ng-if="row.entity.TEAM_LEADER_CONFIRM != \'Y\'" ng-model="row.entity.GUBUN" style="width: 100% !important; height: 100% !important; padding: 0 !important; min-width: 0 !important;" >' + 
...
// 					width : '170',
					...
					allowCellFocus : false,
					cellTemplate : 
						'<div class="ui-grid-cell-contents" ng-if="row.entity.DT_EXIST == \'Y\'" ng-model="row.entity.CONTRACT_NAME" style="text-decoration: underline; font-weight: bold; cursor: pointer; background-color: darkorange;" ng-click="grid.appScope.fn_SELECT_DT(row)">{{row.entity.CONTRACT_NAME}}</div>' +
						'<div class="ui-grid-cell-contents" ng-if="row.entity.DT_EXIST != \'Y\'" ng-model="row.entity.CONTRACT_NAME" style="text-decoration: underline; font-weight: bold; cursor: pointer;" ng-click="grid.appScope.fn_SELECT_DT(row)">{{row.entity.CONTRACT_NAME}}</div>',
...
// 					displayName : '실공수',
					...
					enableCellEdit : false,
					allowCellFocus : false,
					cellTemplate: '<div class="ui-grid-cell-contents">{{grid.appScope.formatter_decimal(row.entity.WORKLOAD_DT)}}</div>',
					aggregationType: uiGridConstants.aggregationTypes.sum,
					footerCellTemplate: '<div class="ui-grid-cell-contents center">{{grid.appScope.formatter_decimal(col.getAggregationValue())}}</div>'
				}, {
					displayName : '보정작업량',
					field : 'COLLECTION_WORKLOAD',
					width : '150',
					visible : true,
					cellClass : "center",
					enableCellEdit : false,
					allowCellFocus : false,
					cellTemplate: 
						'<input type="text" ng-if="row.entity.TEAM_LEADER_CONFIRM != \'Y\'" class="" ng-model="row.entity.COLLECTION_WORKLOAD" style="width: 100%; height: 100% !important; text-align: center;"/>' + 
						'<div ng-if="row.entity.TEAM_LEADER_CONFIRM == \'Y\'" class="ui-grid-cell-contents" ng-model="row.entity.COLLECTION_WORKLOAD" >{{row.entity.COLLECTION_WORKLOAD}}</div>',
					aggregationType: uiGridConstants.aggregationTypes.sum,
					footerCellTemplate: '<div class="ui-grid-cell-contents center">{{grid.appScope.formatter_decimal(col.getAggregationValue())}}</div>'
				}, {
					displayName : '최종작업량',
...
// 					width : '170',
		...
		$(document).ready(function() {
			...
			// EXEC_RFC : "FI"
			var param = {
				listQuery : "yp_zwc_ipt2.select_tbl_working_daily_approval", //list가져오는 마이바티스 쿼리 아이디
				cntQuery : "yp_zwc_ipt2.select_tbl_working_daily_approval_cnt" //list cnt 가져오는 마이바티스 쿼리 아이디
			};
			...
			// EXEC_RFC : "FI"
			var param2 = {
				listQuery : "yp_zwc_ipt2.select_tbl_working_daily_approval_dt", //list가져오는 마이바티스 쿼리 아이디
				cntQuery : "yp_zwc_ipt2.select_tbl_working_daily_approval_dt_cnt" //list cnt 가져오는 마이바티스 쿼리 아이디
			};
...
// 				$("#search_btn").trigger("click");
			...
			$(document).on("click", ".tlc_single", function() {
				...
				$.ajax({
					url : "/yp/zwc/ipt2/update_tbl_working_daily_approval_tlc",
					type : "POST",
					cache : false,
					async : true,
					dataType : "json",
			...
			$("#tlc_y").on("click", function() {
				...
				var header = $("meta[name='_csrf_header']").attr("content");
				$.ajax({
					url : "/yp/zwc/ipt2/update_tbl_working_daily_approval_tlc_y",
					type : "POST",
					cache : false,
					async : true,
					dataType : "json",
			...
			$("#tlc_n").on("click", function() {
				...
				var header = $("meta[name='_csrf_header']").attr("content");
				$.ajax({
					url : "/yp/zwc/ipt2/update_tbl_working_daily_approval_tlc_n",
					type : "POST",
					cache : false,
					async : true,
					dataType : "json",
			...
			$("#fnSave_daily_report").on("click", function() {
				...
				if (confirm("저장하시겠습니까?")) {
					...
					var header = $("meta[name='_csrf_header']").attr("content");
					$.ajax({
						url : "/yp/zwc/ipt2/merge_tbl_working_daily_approval",
						type : "POST",
						cache : false,
						async : true,
						dataType : "json",
...
// 							}
						...
						if(isNaN(d.EXTENSION_HOUR)){
							d.EXTENSION_HOUR = Number("0.0");
						}else{
							if(Number(d.EXTENSION_HOUR) % 1 === 0){
								d.EXTENSION_HOUR = Number(d.EXTENSION_HOUR) + ".0";
							}else{
								d.EXTENSION_HOUR = Number(d.EXTENSION_HOUR) + "";
							}
						}
						if(isNaN(d.HOLIDAY_HOUR)){
							d.HOLIDAY_HOUR = Number("0.0");
						}else{
							if(Number(d.HOLIDAY_HOUR) % 1 === 0){
								d.HOLIDAY_HOUR = Number(d.HOLIDAY_HOUR) + ".0";
							}else{
								d.HOLIDAY_HOUR = Number(d.HOLIDAY_HOUR) + "";
							}
						}
						if(isNaN(d.NIGHT_HOUR)){
							d.NIGHT_HOUR = Number("0.0");
						}else{
							if(Number(d.NIGHT_HOUR) % 1 === 0){
								d.NIGHT_HOUR = Number(d.NIGHT_HOUR) + ".0";
							}else{
								d.NIGHT_HOUR = Number(d.NIGHT_HOUR) + "";
							}
						}
						if(isNaN(d.LATE_HOUR)){
							d.LATE_HOUR = Number("0.0");
						}else{
							if(Number(d.LATE_HOUR) % 1 === 0){
								d.LATE_HOUR = Number(d.LATE_HOUR) + ".0";
							}else{
				...
				if (confirm("저장하시겠습니까?")) {
					...
					var header = $("meta[name='_csrf_header']").attr("content");
					$.ajax({
						url : "/yp/zwc/ipt2/merge_tbl_working_daily_approval_dt",
						type : "POST",
						cache : false,
						async : true,
						dataType : "json",
		...
		function fnValidation(rows){
			var check = true;
			$.each(rows, function(i, d){
				if(isNaN(d.WORKLOAD_DT)){
					d.WORKLOAD_DT = 0;
				}else{
					d.WORKLOAD_DT = Number(d.WORKLOAD_DT);
				}
				if(isNaN(d.COLLECTION_WORKLOAD)){
					d.COLLECTION_WORKLOAD = 0;
				}else{
					d.COLLECTION_WORKLOAD = Number(d.COLLECTION_WORKLOAD);
				}
				if(isNaN(d.WORKLOAD)){
					d.WORKLOAD = 0;
				}else{
