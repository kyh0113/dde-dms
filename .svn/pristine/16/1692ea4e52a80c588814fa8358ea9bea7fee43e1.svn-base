...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if($("input[name=BASE_YYYYMM]").val() == ""){
			$("input[name=BASE_YYYYMM_display]").val("<%=toDay%>");	
...
// 				footerCellTemplate: '<div class="ui-grid-cell-contents center">{{col.getAggregationValue()}}</div>'
	...
	$(document).ready(function(){
		...
		var param = {
				EXEC_RFC  : "N", // RFC 여부
				cntQuery  : "yp_zcs_ipt.select_construction_monthly_rpt3_cnt", 	
				listQuery : "yp_zcs_ipt.select_construction_monthly_rpt3"
		};
		...
		$("#excelTemplate_btn").on("click",function(){
	    	...
	    	input.value = "mon_create_sample3.xlsx";
	    	input.type = "hidden";
			...
			swal({
				...
				.then(function(result){
				  if(result){
						...
						$.ajax({
							url : "/yp/zcs/ipt/zcs_ipt_mon_create_save",
							type : "POST",
							cache : false,
							async : true,
							dataType : "json",
...
//					data[i]["value"] = data[i]["value"].replace(/ /gi, "");// 모든 공백을 제거
		...
		$.ajax({
			url : "/yp/zcs/ipt/month_excelUpload_check",
			type : "POST",
			cache : false,
			async : false, //동기화
			dataType : "json",
			data : data,
			success : function(result) {
				isExistMonthlyRpt3 = result.result;
			},
			beforeSend : function(xhr) {
				// 2019-10-23 khj - for csrf
				xhr.setRequestHeader(header, token);
