...
// 				'TEAM' : "성우공업사"
			...
			$.ajax({
				url : "/yp/zcs/ipt/select_zcs_ipt_mon_manh_create",
				type : "POST",
				cache : false,
				async : true,
				dataType : "json",
...
// 						innerHtml += '	<td class="right vertical-center"><div style="padding: 4px 3px;">' + ( manhour_sum - manhour_cnt ) + '</div></td>';
		...
		$("#add").on("click", function() {
			...
			var innerHtml = '';
			innerHtml += '<tr class="data-base" data-rn="' + ( $(".data-base:last").data("rn") + 1 ) + '">';
			innerHtml += '	<td class="center vertical-center"	style="width: 33px;">';
			...
			innerHtml += '			DBYN="N"';
			innerHtml += '			BASE_YYYYMM="" ';
			innerHtml += '			VENDOR_CODE="" ';
			innerHtml += '			CONTRACT_CODE="" ';
			innerHtml += '			SEQ="" ';
			innerHtml += '			ORDER_NUMBER="" ';
			innerHtml += '			WORK_CONTENTS="" ';
			innerHtml += '			COST_CODE="" ';
			innerHtml += '			COST_NAME="" ';
			innerHtml += '			WORK_START_DT="" ';
			innerHtml += '			WORK_START_TIME="" ';
			innerHtml += '			WORK_END_DT="" ';
			...
			innerHtml += '		/>';
			innerHtml += '	</td>';
			innerHtml += '	<td class="left vertical-center"	style="width: 160px;">';
			...
			innerHtml += '	</td>';
			innerHtml += '	<td class="left vertical-center"	style="width: 10%;">	<input type="text"	class="rows-no-' + ( $(".data-base:last").data("rn") + 1 ) + ' cost_name" value="" style="width: 100%;" readonly="readonly"/></td>';
			...
			innerHtml += '	<td class="center vertical-center"	style="width: 130px;">	<input type="text"	class="calendar dtp_ymd" data-rn="' + ( $(".data-base:last").data("rn") + 1 ) + '" onchange="javascript: reflectCommon(this, \'WORK_START_DT\');" style="width: 100%;" readonly="readonly"/></td>';
			...
			innerHtml += '	<td class="center vertical-center"	style="width: 130px;">	<input type="text"	class="calendar dtp_ymd" data-rn="' + ( $(".data-base:last").data("rn") + 1 ) + '" onchange="javascript: reflectCommon(this, \'WORK_END_DT\');" style="width: 100%;" readonly="readonly"/></td>';
		...
		$("#remove").on("click", function() {
			...
			if (confirm("삭제하겠습니까?")) {
				...
				if( delete_datas.length > 0 ){
					...
					var header = $("meta[name='_csrf_header']").attr("content");
					$.ajax({
						url : "/yp/zcs/ipt/delete_zcs_ipt_mon_manh_create",
						type : "POST",
						cache : false,
						async : false,
						dataType : "json",
						...
						success : function(result) {
							swalSuccessCB("삭제 완료됐습니다.", function(){
		...
		$("#save").on("click", function() {
			...
			if(fnValidation()){
				if (confirm("저장하시겠습니까?")) {
					...
					var header = $("meta[name='_csrf_header']").attr("content");
					$.ajax({
						url : "/yp/zcs/ipt/merge_zcs_ipt_mon_manh_create",
						type : "POST",
						cache : false,
						async : false,
						dataType : "json",
						...
						success : function(data) {
							console.log(data);
							swalSuccessCB("저장 완료됐습니다.", function(){
								$("#search_btn").trigger("click");
							});
						},
						beforeSend : function(xhr) {
