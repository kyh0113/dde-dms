...
//		}
	...
	function fnSendData(i){		
		...
		$("input[name=REPORT_CODE]", opener.document).val($("input[name=REPORT_CODE_INPUT]:eq("+i+")").val());		//보고서코드 
...
<body>
	<div id="popup">
		<div class="pop_header">보고서 검색</div>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="REPORT_CODE" <c:if test="${req_data.search_type eq 'REPORT_CODE'}">selected</c:if>>보고서코드</option>
					</select>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>${list.REPORT_CODE}<input type="hidden" name="REPORT_CODE_INPUT" value="${list.REPORT_CODE}"></td>
									<td>${list.VENDOR_NAME}<input type="hidden" name="VENDOR_NAME_INPUT" value="${list.CONTRACT_NAME}"></td>
