	...
	<form id="frm" name="frm">
		...
		<section class="section">
			...
			<div class="tab-content tbl_box">
				...
				<!-- 작업내역 -->
				<div class="tab-pane" id="work_history_content" style="overflow:auto;">
					<div class="tbl_box" style="background: white;">
						<!-- 복붙영역(html) 시작, 복붙시 바꿔야할 값들 : id, data-ng-controller  -->
						<div id="shds2-uiGrid" data-ng-controller="shds2Ctrl">
							<div data-ui-i18n="ko" style="height: 480px;">
...
<script>
	...
	$(document).ready(function(){
			...
			swal({
				...
				.then(function(result){
				  if(result){
						...
						$.ajax({
							url : "/yp/zcs/cpt/zcs_cpt_opt_create_save",
							type : "POST",
							cache : false,
							async : true,
							dataType : "json",
		...
		if(!isEmpty('${REPORT_CODE}')){
			...
			$.ajax({
				url : "/yp/zcs/cpt/zcs_cpt_opt_create_select",
				type : "post",
				cache : false,
				async : false,//동기화
				data : data,
...
//					data[i]["value"] = data[i]["value"].replace(/ /gi, "");// 모든 공백을 제거
		...
		//1:거래처,계약코드,월보년월로 데이터가져오기
		if(flag == '1'){
			url = "/yp/zcs/cpt/select_zcs_cpt_opt_create"
		//2:보고서코드로 데이터가져오기
		}else if(flag == "2"){
			url = "/yp/zcs/cpt/select_zcs_cpt_opt_create2"
		...
		$.ajax({
			...
			success : function(result) {
				//상단
				var inspection_opt_list1 = result.inspection_opt_list1;
				//하단
				var inspection_opt_list2 = result.inspection_opt_list2;
				//construction_rpt_chk
				var construction_chk_rpt = result.construction_chk_rpt;
				
				//console.log('[TEST]inspection_opt_list1:',inspection_opt_list1);
				//console.log('[TEST]inspection_opt_list2:',inspection_opt_list2);
				...
				if(flag == '1'){
					...
					if(inspection_opt_list2.length == 0 || isEmpty(inspection_opt_list2[0].VENDOR_CODE) || isEmpty(inspection_opt_list2[0].CONTRACT_CODE) || isEmpty(inspection_opt_list2[0].BASE_YYYYMM)){
						//table tr 초기화
				...
				if(!isEmpty(inspection_opt_list1)){
					//$("input[name=CONTRACT_CODE_RESULT]").val(inspection_opt_list1[0].CONTRACT_CODE);
					//$("input[name=CONTRACT_NAME_RESULT]").val(inspection_opt_list1[0].CONTRACT_NAME);
					//$("input[name=VENDOR_CODE_RESULT]").val(inspection_opt_list1[0].VENDOR_CODE);
					//$("input[name=VENDOR_NAME_RESULT]").val(inspection_opt_list1[0].VENDOR_NAME);
					$("input[name=WBS_CODE1]").val(inspection_opt_list1[0].WBS_CODE1);
					$("input[name=WBS_CODE2]").val(inspection_opt_list1[0].WBS_CODE2);
					$("input[name=WBS_CODE3]").val(inspection_opt_list1[0].WBS_CODE3);
					$("input[name=WBS_CODE4]").val(inspection_opt_list1[0].WBS_CODE4);
					$("input[name=WBS_CODE5]").val(inspection_opt_list1[0].WBS_CODE5);
					$("input[name=WBS_CODE6]").val(inspection_opt_list1[0].WBS_CODE6);
					//$("input[name=BASE_YYYYMM_RESULT]").val(YYYYMM_format(inspection_opt_list2[0].BASE_YYYYMM));
					
					//---------------- 상단 테이블 데이터 넣어주기 ---------------------------
					//상단 : 맨위부터 ~ 공정구분행 까지.
					var WBS_CODE = inspection_opt_list1[0].WBS_CODE;
					var CONTRACT_NAME = inspection_opt_list1[0].CONTRACT_NAME;
					var VENDOR_NAME = inspection_opt_list1[0].VENDOR_NAME;
				...
				for(var i=0; i<inspection_opt_list2.length; i++){
					var obj = inspection_opt_list2[i];
					var innerHtml = "";
					...
					if(BEFORE_COST_CODE != CURRENT_COST_CODE){
						...
						//대표 COST_NAME 넣어주기
						innerHtml = '<tr>';
						innerHtml += '	<td colspan="7" class="left vertical-center tb-head" ><span style="color: #000; background-color: #fff; padding: 5px;">'+(++order)+'. '+obj.COST_NAME+'</span></td>';
		    			innerHtml += '</tr>';
					...
					innerHtml = '<tr>';
					innerHtml += '	<td class="left vertical-center tb-head" >'+obj.CONTENTS+'</td>';
					innerHtml += '	<td class="center vertical-center" >'+obj.RATE_AMOUNT+'</td>';
					innerHtml += '	<td class="center vertical-center" >'+obj.MARK_UNIT+'</td>';
					innerHtml += '	<td class="center vertical-center" >'+obj.QTY+'</td>';
		    		...
		    		if(i == inspection_opt_list2.length-1){
		    			//소계 만들어주기
		    			innerHtml = '<tr class="sub_total ">';
		    			innerHtml += '	<td colspan="5" class="center vertical-center">소계</td>';
		    			innerHtml += '	<td class="right vertical-center">'+addComma(sub_total_amount)+'</td>';	//소계 금액
	...
	function work_status(flag){
		...
		//1:거래처,계약코드,월보년월로 데이터가져오기
		if(flag == '1'){
			url = "/yp/zcs/cpt/select_work_status_opt"
		//2:보고서코드로 데이터가져오기
		}else if(flag == "2"){
		...
		$.ajax({
			...
			success : function(result) {
				...
				//console.log("메인",work_status_list);
				//날짜별 합계 map
				var sum_by_date_map = result.sum_by_date_map;
				
				//코스트센터별 소계 리스트
				var sum_by_cost_list = result.sumByCostList;
				//console.log("소계",sum_by_cost_list);
				var loop_no = 0;	//소계 리스트 index
				...
				//총합계
				var total_sum = 0;
				//코스트 코드를 기준으로 소계 만들기 위한 변수
				var BEFORE_COST = "";

				//-------------------데이터 보여주기-----------------------
				for(var i = 0; i < work_status_list.length; i++){

					if(i==0) BEFORE_COST = work_status_list[i].COST_CODE;	//루프 최초시 현재 코스트센터 담기
					var CURRENT_COST = work_status_list[i].COST_CODE;		//현재 코스트센터 담기
...
// 					if(cost_sum !== 0){
						...
						//소계 ROW 생성
						if(BEFORE_COST != CURRENT_COST){	//코스트센터명 비교
							
							//소계 넣어주기
							innerHtml += '<tr>'
							innerHtml += '<td colspan="2" class="center vertical-center" style="background-color:#ccc;" nowrap>소계</td>'
							
							var sub_cost_total = 0;
							for(var key in sum_by_cost_list[loop_no]){
					    		//날짜별 합계 만들어주기
								if(!isNaN(key)){
									var tot = sum_by_cost_list[loop_no][key];	//코스트센터별 소계
									sub_cost_total += tot;	//코스트센터별 소계 SUM
									//if(tot == 0) tot = '';	//코스트센터별 소계가 0이면 공백으로 보여줌
									innerHtml += '	<td class="center vertical-center" style="background-color:#ccc;">'+tot+'</td>';	//코스트센터별 소계
					    		}
							}
							innerHtml += '	<td class="center vertical-center" style="background-color:#ccc;">'+sub_cost_total+'</td>';	//코스트센터별 소계 토탈
							innerHtml += '</tr>'
							$('#work_status_table').append(innerHtml);	//소계 삽입
							loop_no++;
						}
						...
						for(var key in work_status_list[i]){
							var obj = work_status_list[i];
							if(key=="COST_NAME"){
								innerHtml += '<td class="center vertical-center" nowrap>'+obj[key]+'</td>'
						...
						innerHtml += '<td class="center vertical-center" >'+cost_sum+'</td>';
						innerHtml += '</tr>';
						
						//마지막 row 소계 넣기
			    		if(i == work_status_list.length-1){
							//소계 넣어주기
							innerHtml += '<tr>';
							innerHtml += '<td colspan="2" class="center vertical-center" style="background-color:#ccc;" nowrap>소계</td>';
...
// 					}
						...
						BEFORE_COST = CURRENT_COST;	//현재 코스트센터 셋
						innerHtml = "";	//초기화
				}
				//----------------------------------------------------
				
				//-----------------날짜별 합계 & 총합계 보여주기(Footer 계)--------------
				innerHtml = '<tr class="sum_total">';
					innerHtml += '	<td class="center vertical-center" colspan="2">계</td>';
					for(var key in work_status_list[0]){
		    			//날짜별 합계 만들어주기
						if(!isNaN(key)){
							innerHtml += '	<td class="center vertical-center" >'+sum_by_date_map[key]+'</td>';
		    			}
					}
				//총합계
				innerHtml += '	<td class="center vertical-center" >'+total_sum+'</td>';
				innerHtml += '</tr>';
				$('#work_status_table').append(innerHtml);
				//----------------------------------------------------
			},
