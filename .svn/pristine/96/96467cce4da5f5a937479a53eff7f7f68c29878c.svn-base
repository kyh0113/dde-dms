...
<script type="text/javascript">
	...
	function fnSendData(i){		
		...
		console.log(i);
		if("${req_data.target}" == ""){
			$("input[name=POSID]", opener.document).val($("input[name=POSID]:eq("+i+")").val());		//WBS코드
		}else{
			opener.scope.gridOptions.data[target].POSID = $("input[name=POSID]:eq("+i+")").val();		//WBS코드
			opener.scope.gridOptions.data[target].POST1 = $("input[name=POST1]:eq("+i+")").val();		//WBS코드명
...
<body>
	<div id="popup">
		<div class="pop_header">WBS 검색</div>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>${list.POSID}<input type="hidden" name="POSID" value="${list.POSID}">
									</td>
									<td>${list.POST1}<input type="hidden" name="POST1" value="${list.POST1}">
									</td>
								</tr>
							</c:forEach>
						</c:if>
