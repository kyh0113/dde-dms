...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		var cd = $("#VAPLZ", opener.document);
		var nm = $("#VATXT", opener.document);
		if( cd.val().trim() === "" ){
			cd.val( $("input[name=ARBPL]:eq("+i+")").val() );	//코스트센터코드
			nm.val( $("input[name=KTEXT]:eq("+i+")").val() );		//코스트센터
		}else{
			cd.val( cd.val().trim() + "," + $("input[name=ARBPL]:eq("+i+")").val() );
			nm.val( nm.val().trim() + "," + $("input[name=KTEXT]:eq("+i+")").val() );
		}
	}
</script>
</head>
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="1">작업장코드</option>
						<option value="2" selected="selected">작업장</option>
					</select>
					...
					<c:if test="${fn:length(list) > 0}">
						<c:forEach var="list" items="${list}" varStatus="i">
							<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
								...
								<td>
									${list.ARBPL}<input type="hidden" name="ARBPL" value="${list.ARBPL}">
								</td>
								<td>
