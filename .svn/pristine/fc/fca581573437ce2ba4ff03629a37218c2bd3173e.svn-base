...
<body>
			...
			<table class="contract_standard_table" cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<td>
						...
						<input type="hidden" id="VENDOR_CODE" name="VENDOR_CODE" size="10" value="${ent_vendor_code}"/>
	...
	<section class="section">
		...
		<div id="shds-uiGrid" data-ng-controller="shdsCtrl">
			<div data-ui-i18n="ko" style="height: 585px;">
	...
	app.controller('shdsCtrl', [ '$scope', '$window', '$controller', '$log', 'StudentService', 'uiGridConstants', function($scope, $window, $controller, $log, StudentService, uiGridConstants) { //$scope(this)는 해당컨트롤러로 진입하기위한 접근지시자라고 보면됨
		...
		//일보승인 페이지 이동
		$scope.link_rpt = function(row) {
			var u, h, cc, cn, vc, vn, ec, wd, cpc, drpc;
			if(row.PAY_STANDARD === "1"){
				u = "/yp/zcs/ipt2/zcs_ipt2_daily_rpt1";
			}else if(row.PAY_STANDARD === "2"){
				u = "/yp/zcs/ipt2/zcs_ipt2_daily_rpt2";
			}else if(row.PAY_STANDARD === "3"){
				u = "/yp/zcs/ipt2/zcs_ipt2_daily_rpt3";
			}else{
				return false;
			}
			f_href(u, {
		...
		var columnDefs1 = [ 
			{
				...
				enableCellEdit : false,
				allowCellFocus : false,
				cellTemplate : '<div class="ui-grid-cell-contents relatvie_box height_full pointer" style="color:blue" ng-dblclick="grid.appScope.link_rpt(row.entity)" ng-model="row.entity.WORK_DT_VIEW">{{row.entity.WORK_DT_VIEW}}</div>'
			}, {
				displayName : '지급기준',
