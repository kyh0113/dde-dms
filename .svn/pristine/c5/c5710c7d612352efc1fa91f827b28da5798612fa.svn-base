...
<body>
			...
			<div class="tab-pane daily_rpt2_content" id="daily_rpt2">
				<div class="tbl_box" style="background: none;">
					<table border="1" class="table">
						...
						<tr style="background-color: white;"><td class="center vertical-center" colspan="15">데이터가 없습니다.</td></tr>
					</table>
				</div>
			</div>
		</div>
	...
	$(document).ready(function(){
		...
		$("#search_btn").on("click", function() {
			...
			$.ajax({
				url : "/yp/zcs/ipt2/select_zcs_ipt2_daily_aprv2",
				type : "POST",
				cache : false,
				async : true,
				dataType : "json",
				...
				success : function(data) {
					...
					// 태그데이터
					if(result1.length === 0){
						innerHtml = '<tr style="background-color: white;"><td class="center vertical-center" colspan="15">데이터가 없습니다.</td></tr>';
					}
...
// 						return false;
					...
					$.each(result2, function(i, d){
						...
						innerHtml += '	<td class="center vertical-center"	style="width: 50px;"><div style="padding: 4px 3px;">' + d.VIEW_M + '</div></td>';
						innerHtml += '	<td class="center vertical-center"	style="width: 50px;"><div style="padding: 4px 3px;">' + d.VIEW_D + '</div></td>';
						innerHtml += '	<td class="center vertical-center"	style="width: 50px;"><div style="padding: 4px 3px;">' + d.WORK_START_TIME + '</div></td>';
						innerHtml += '	<td class="center vertical-center"	style="width: 50px;"><div style="padding: 4px 3px;">' + d.WORK_END_TIME + '</div></td>';
						innerHtml += '	<td class="center vertical-center"	style="width: 80px;"><div style="padding: 4px 3px;">' + d.COST_CODE + '</div></td>';
		...
		/* 기한설정, 기한해제 - 작업 */
		$(".daily_work_view #btn_lock, .daily_work_view #btn_release").on("click", function() {
			var exec = $(this).data("exec");
			var exec_msg = ( exec === "L" ? "기한설정" : "기한해제" );
			var exec_url = ( exec === "L" ? "/yp/zcs/ipt2/lock_zcs_ipt2_daily_aprv2" : "/yp/zcs/ipt2/release_zcs_ipt2_daily_aprv2" );
			if( fnValidation_limit_work( exec_msg ) ){
				swal({
					icon : "info",
		...
		/* 기한설정, 기한해제 - 작업 */
		$(".daily_rpt2_view #btn_lock, .daily_rpt2_view #btn_release").on("click", function() {
			var exec = $(this).data("exec");
			var exec_msg = ( exec === "L" ? "기한설정" : "기한해제" );
			var exec_url = ( exec === "L" ? "/yp/zcs/ipt2/lock_zcs_ipt2_daily_aprv2_tag" : "/yp/zcs/ipt2/release_zcs_ipt2_daily_aprv2_tag" );
		...
		$(".daily_work_view #btn_confirm, .daily_work_view #btn_cancel").on("click", function() {
			...
			var exec = $(this).data("exec");
			if(fnValidation_confirm_work( ( exec === "CF" ? "승인" : "승인취소" ) )){
				...
				swal({
					...
					if(result){
						...
						var header = $("meta[name='_csrf_header']").attr("content");
						$.ajax({
							url : ( exec === "CF" ? "/yp/zcs/ipt2/update_aprv_zcs_ipt2_daily_aprv2" : "/yp/zcs/ipt2/update_reject_zcs_ipt2_daily_aprv2" ),
							type : "POST",
							cache : false,
							async : false,
							dataType : "json",
			...
			if(fnValidation_confirm( ( exec === "CF" ? "승인" : "승인취소" ) )){
				...
				swal({
					...
					if(result){
						...
						var header = $("meta[name='_csrf_header']").attr("content");
						$.ajax({
							url : ( exec === "CF" ? "/yp/zcs/ipt2/update_aprv_zcs_ipt2_daily_aprv2_tag" : "/yp/zcs/ipt2/update_reject_zcs_ipt2_daily_aprv2_tag" ),
							type : "POST",
							cache : false,
							async : false,
							dataType : "json",
	...
	function fnValidation_limit_work(msg){
...
// 			console.log(i, d);
	...
	function reflectFooter() {
		...
		var addition_col2 = 0;
		var addition_col3 = 0;
		$('.daily_rpt2_content .data.data-col-2').each(function(i, d){
			addition_col2 += Number( $(d).val() );
		});
		$('.daily_rpt2_content .sub1.data-col-2').val(addition_col2.toFixed(2));
