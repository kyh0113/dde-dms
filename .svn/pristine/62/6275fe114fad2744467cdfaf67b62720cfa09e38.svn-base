...
<script type="text/javascript">
	...
	function fnSendData(i){
		...
		if(type == "srch"){
			if(target >= 0){
				opener.scope.gridOptions.data[target].ENT_CODE = $("input[name=KUNNR]:eq("+i+")").val();
				opener.scope.gridOptions.data[target].ENT = $("input[name=NAME1]:eq("+i+")").val();
				opener.scope.gridOptions.data[target].STCD2 = $("input[name=STCD2]:eq("+i+")").val();
				opener.scope.gridApi.grid.refresh();
			}else{
				$("input[name=srch_ent_code]", opener.document).val($("input[name=KUNNR]:eq("+i+")").val());
				$("input[name=srch_ent]", opener.document).val($("input[name=NAME1]:eq("+i+")").val());
				$("input[name=STCD2]", opener.document).val($("input[name=STCD2]:eq("+i+")").val());
			}
		}else if(type == "SO"){
			$("input[name=I_PART_PARTN_NUMB]", opener.document).val($("input[name=KUNNR]:eq("+i+")").val());
			$(".I_PART_PARTN_NUMB", opener.document).text($("input[name=NAME1]:eq("+i+")").val());
		}else{
			$("input[name=ent_code]", opener.document).val($("input[name=KUNNR]:eq("+i+")").val());
			$("input[name=ent]", opener.document).val($("input[name=NAME1]:eq("+i+")").val());
			$("input[name=STCD2]", opener.document).val($("input[name=STCD2]:eq("+i+")").val());
		}
		self.close();
	}
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="I_NAME1" <c:if test="${req_data.search_type eq 'I_NAME1'}">selected</c:if>>설명</option>
						<option value="I_KUNNR" <c:if test="${req_data.search_type eq 'I_KUNNR'}">selected</c:if>>코드</option>
					</select>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>
										<fmt:parseNumber value="${list.KUNNR}" />
										<input type="hidden" name="KUNNR" value="<fmt:parseNumber value="${list.KUNNR}"/>">
									</td>
									<td style="text-align: left;">${list.NAME1}<input type="hidden" name="NAME1" value="${list.NAME1}">
									</td>
									<input type="hidden" name="STCD2" value="${list.STCD2}">
								</tr>
							</c:forEach>
						</c:if>
						<c:if test="${fn:length(list) == 0}">
