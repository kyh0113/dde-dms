...
<body>
	...
	<!-- 엑셀업로드 insertQuery -->
	<input type='hidden' id='insertQuery' name='insertQuery' value='oracle.yp_zcs_ipt.create_construction_monthly_rpt2'>
		...
		<div class="tab-content tbl_box">
			<div class="tab-pane daily_work_content show active" id="daily_work">
				<div class="tbl_box" style="background: none;">
					<table border="1" class="table">
						<tr class="tb-head">
							<td style="color: white; width: 33px;"><input type="checkbox" style="zoom: 1.5; display: none;" class="all-check" onchange="javascript: batch_checkbox(this,1);"></td>
						...
						<tr style="background-color: white;"><td class="center vertical-center" colspan="12">데이터가 없습니다.</td></tr>
					</table>
				</div>
			</div>
			<div class="tab-pane daily_rpt2_content" id="daily_rpt2">
				<div class="tbl_box" style="background: none;">
					<table border="1" class="table">
						<tr class="tb-head">
							<td style="color: white; width: 33px;"><input type="checkbox" style="zoom: 1.5; display: none;" class="all-check" onchange="javascript: batch_checkbox(this,2);"></td>
							<td style="color: white; width: 50px;">저장<br>여부</td>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			var param = {
				EXEC_RFC : "N", // RFC 여부
				cntQuery : "yp_zcs_ipt2.select_construction_monthly_rpt2_cnt",
				listQuery : "yp_zcs_ipt2.select_construction_monthly_rpt2"
			};
			...
			$("#search_btn").on("click", function() {
				...
				$.ajax({
					url : "/yp/zcs/ipt2/select_zcs_ipt2_month_rpt2",
					type : "POST",
					cache : false,
					async : true,
					dataType : "json",
					...
					success : function(data) {
						...
						if (result1.length === 0) {
							innerHtml = '<tr style="background-color: white;><td class="center vertical-center" colspan="12">데이터가 없습니다.</td></tr>';
...
// 							innerHtml += '		<select data-rn="' + i + '" onchange="javascript: reflectCombo(this);" style="width: 100%;">';
// 							var loop_combo_html = '	<option value="">= 선택 =</option>';
// 							$.each(result3, function(i, sd){
// 								loop_combo_html += '	<option value="' + sd.CONTENTS + '" ' + ( d.WORK_CONTENTS === sd.CONTENTS ? 'selected' : '' ) + '>' + sd.CONTENTS + '</option>';
// 							});
// 							innerHtml += 			loop_combo_html;
// 							innerHtml += '		</select>';
// 							innerHtml += '	</td>';
							innerHtml += '	<td class="center vertical-center"	style="width: 120px;"><div style="padding: 4px 3px;">' + d.ORDER_NUMBER + '</div></td>';
							innerHtml += '	<td class="right vertical-center"	style="width: 100px;"><div style="padding: 4px 3px;">' + d.QTY + '</div></td>';
							innerHtml += '</tr>';
						});
						$('.daily_work_content .table').append(innerHtml);
					},
			...
			$(".daily_work_view #remove").on("click", function() {
				...
				$(".daily_work_content .data-base input.rows-checkbox").each(function(i, obj) {
					if ($(obj).attr("DBYN") !== "Y") {
						chk_cnt++;
						return false;
					}else{
				...
				if( chk_cnt > 0 || delete_datas.length === 0 ){
					swalWarningCB("저장된 【작업】항목 월보가 없습니다.");
					return false;
				}
				// DB 삭제 대상 없으면 종료
				...
				swal({
					...
					if(result){
						if (delete_datas.length > 0) {
							...
							var header = $("meta[name='_csrf_header']").attr("content");
							$.ajax({
								url : "/yp/zcs/ipt2/delete_zcs_ipt2_month_rpt2",
								type : "POST",
								cache : false,
								async : false,
								dataType : "json",
								...
								success : function(data) {
									if(data.result > 0){
										swalSuccessCB("【작업】항목 【삭제】완료되었습니다.", function() {
			...
			$(".daily_rpt2_view #remove").on("click", function() {
				...
				if( chk_cnt > 0 || delete_datas.length === 0 ){
					swalWarningCB("저장된 【태그데이터】항목 월보가 없습니다.");
					return false;
				}
				// DB 삭제 대상 없으면 종료
				if (delete_datas.length > 0) {
				...
				swal({
					...
					if(result){
						if (delete_datas.length > 0) {
							...
							var header = $("meta[name='_csrf_header']").attr("content");
							$.ajax({
								url : "/yp/zcs/ipt2/delete_zcs_ipt2_month_rpt2_tag",
								type : "POST",
								cache : false,
								async : false,
								dataType : "json",
								...
								success : function(data) {
									if(data.result > 0){
										swalSuccessCB("【태그데이터】항목 【삭제】완료되었습니다.", function() {
											$("#search_btn").trigger("click");
										});
									}else{
										$("#search_btn").trigger("click");
								...
								error : function(request, status, error) {
									console.log("code:" + request.status + "\n" + "message:" + request.json + "\n" + "error:" + error);
									swalDangerCB("【태그데이터】항목 【삭제】실패하였습니다.\n관리자에게 문의해주세요.");
								}
							});
						}
					}
				...
				if (fnValidation_work()) {
					swal({
						...
						if(result){
							...
							var header = $("meta[name='_csrf_header']").attr("content");
							$.ajax({
								url : "/yp/zcs/ipt2/merge_zcs_ipt2_month_rpt2",
								type : "POST",
								cache : false,
								async : false,
								dataType : "json",
								...
								success : function(data) {
									console.log(data);
									swalSuccessCB("【작업】항목 【저장】완료되었습니다.", function() {
										$("#search_btn").trigger("click");
									});
								},
								beforeSend : function(xhr) {
				...
				if (fnValidation()) {
					swal({
						...
						if(result){
							...
							var header = $("meta[name='_csrf_header']").attr("content");
							$.ajax({
								url : "/yp/zcs/ipt2/merge_zcs_ipt2_month_rpt2_tag",
								type : "POST",
								cache : false,
								async : false,
								dataType : "json",
								...
								success : function(data) {
									console.log(data);
									swalSuccessCB("【태그데이터】항목 【저장】완료되었습니다.", function() {
										$("#search_btn").trigger("click");
									});
								},
								beforeSend : function(xhr) {
								...
								error : function(request, status, error) {
									console.log("code:" + request.status + "\n" + "message:" + request.json + "\n" + "error:" + error);
									swalDangerCB("【태그데이터】항목 【저장】실패하였습니다.\n관리자에게 문의해주세요.");
								}
							});
						}
					});
