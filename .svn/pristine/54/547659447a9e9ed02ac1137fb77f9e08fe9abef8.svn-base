...
<script>
	$(document).ready(function(){
		...
		var param = {
			listQuery : "yp_wr.grid_wr_writehist_list", 				//list가져오는 마이바티스 쿼리 아이디
			cntQuery : "yp_wr.grid_wr_writehist_list_cnt"		//list cnt 가져오는 마이바티스 쿼리 아이디
		}; 
	...
	function ($scope, $controller, $log, StudentService, uiGridConstants) {		//$scope(this)는 해당컨트롤러로 진입하기위한 접근지시자라고 보면됨
		...
		$scope.gridOptions = vm.gridOptions( // 그리드 옵션, 부모의 그리드 옵션에 파라미터를 던지면서 변경해서 대입
		{
			...
			},
			{
				 ...
				 */
				cellTemplate : 
					'<div ng-if="!grid.appScope.isApproval(row.entity.EDOC_STATUS)" class="ui-grid-cell-contents btn_g"  style="cursor: pointer;" ng-click="grid.appScope.fnApprovalC(row)">결재상신</div>'
