...
<body>
			...
			<div class="tbl_box">
				<table cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<th>부서명</th>
						<td>
							<input type="text" disabled="disabled" name="dept_name" value="${req_data.dept_name}">
						</td>
					</tr>
				</table>
				<div class="btn_wrap">
	...
	<script>
		...
		app.controller('shdsCtrl', [ '$scope', '$window', '$controller', '$log', 'StudentService', 'uiGridConstants', function($scope, $window, $controller, $log, StudentService, uiGridConstants) { //$scope(this)는 해당컨트롤러로 진입하기위한 접근지시자라고 보면됨
			...
			var columnDefs1 = [
				...
				},
				{
					...
					enableCellEdit : false,
					allowCellFocus : false,
					cellTemplate : '<div class="ui-grid-cell-contents"><span ng-if="row.entity.MEAL_TYPE ==\'1\'">조식</span><span ng-if="row.entity.MEAL_TYPE ==\'2\'">중식</span><span ng-if="row.entity.MEAL_TYPE ==\'3\'">석식</span><span ng-if="row.entity.MEAL_TYPE ==\'4\'">야식</span><br/></div>'
				}, 
				{
					displayName : '부서명',
					field : 'DEPT_NAME',
		...
		$(document).ready(function() {
			...
			var param = {
					EXEC_RFC  : "N", // RFC 여부
					cntQuery  : "yp_ZHR.retrieveDosilakCheckListCnt", 	
					listQuery : "yp_ZHR.retrieveDosilakCheckList"
			};
