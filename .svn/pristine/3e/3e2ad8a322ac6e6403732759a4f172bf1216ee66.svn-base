...
<script type="text/javascript">
	$(document).ready(function() {
		$("#BUDAT").val($("input[name=BUDAT]", opener.document ).val());
			...
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<div class="tbl_box">
					<select name="search_type">
						<option value="I_WAERS">통화</option>
						<option value="I_LTEXT" selected>통화설명</option>
					</select>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>${list.WAERS}<input type="hidden" name="WAERS" value="${list.WAERS}">
									</td>
									<td>
										${list.LTEXT}
										<input type="hidden" name="LTEXT" value="${list.LTEXT}">
										<input type="hidden" name="UKURS" value="${list.UKURS}">
									</td>
								</tr>
							</c:forEach>
						</c:if>
