...
<script type="text/javascript">
	$(document).ready(function() {
		$("input[name=I_AGKOA]").val($("input[name=AGKOA]",opener.document).val());
	...
	function fnSendData(i){		
		...
		opener.console.log(i);
		
		opener.scope.gridOptions.data[target].SAP_CODE = $("input[name=LIFNR]:eq("+i+")").val();
		opener.scope.gridOptions.data[target].CODE_NAME = $("input[name=NAME1]:eq("+i+")").val();
	...
	<div id="popup">
		...
		<div class="pop_content">
			<form id="frm" name="frm" method="post" accept-charset="UTF-8">
				...
				<input type="hidden" name="I_AGKOA" value="" />
