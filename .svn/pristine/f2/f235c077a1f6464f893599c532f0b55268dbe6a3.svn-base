...
<title>Seller 성분분석 결과 비교 [현재는 사용하지 않음]</title>
</head>
<body>
	...
	<div class="float_wrap" style="margin-bottom: 2px;">
		<div class="fl">
			...
			<input type="text" id="split_limit_1" >
		</div>
		<div class="fr">
			<div class="btn_wrap">
				<input type=button class="btn_g" id="fnReg1" value="등록 완료">
			</div>
		</div>
	</div>
	...
	<section class="section">
		<!-- 복붙영역(html) 시작, 복붙시 바꿔야할 값들 : id, data-ng-controller  -->
		<div id="data-uiGrid-1" data-ng-controller="dataCtrl-1" style="height: auto;">
			<div data-ui-i18n="ko" style="height: 220px;">
				<div data-ui-grid="gridOptions" class="grid" data-ui-grid-edit data-ui-grid-cellNav ui-grid-pagination data-ui-grid-resize-columns data-ui-grid-auto-resize ui-grid-auto-resize data-ui-grid-selection data-ui-grid-exporter ui-grid-pinning>
					<div data-ng-if="loader" class="loader"></div>
					<div class="watermark" data-ng-show="!gridOptions.data.length">데이터가 없습니다.</div>
	...
	<div class="float_wrap" style="margin-bottom: 2px;">
		<div class="fl">
			<h8>성분2 : Au</h8>
			<label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Split Limit : </label>
			<input type="text" id="split_limit_2" >
		</div>
		<div class="fr">
			<div class="btn_wrap">
				<input type=button class="btn_g" id="fnReg2" value="등록 완료">
			</div>
		</div>
	</div>
	...
	<section class="section">
		<!-- 복붙영역(html) 시작, 복붙시 바꿔야할 값들 : id, data-ng-controller  -->
		<div id="data-uiGrid-2" data-ng-controller="dataCtrl-2" style="height: auto;">
			<div data-ui-i18n="ko" style="height: 220px;">
				<div data-ui-grid="gridOptions" class="grid" data-ui-grid-edit data-ui-grid-cellNav ui-grid-pagination data-ui-grid-resize-columns data-ui-grid-auto-resize ui-grid-auto-resize data-ui-grid-selection data-ui-grid-exporter ui-grid-pinning>
					<div data-ng-if="loader" class="loader"></div>
					<div class="watermark" data-ng-show="!gridOptions.data.length">데이터가 없습니다.</div>
	...
	<div class="float_wrap" style="margin-bottom: 2px;">
		<div class="fl">
			<h8>성분3 : Ag</h8>
			<label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Split Limit : </label>
			<input type="text" id="split_limit_3" >
		</div>
		<div class="fr">
			<div class="btn_wrap">
				<input type=button class="btn_g" id="fnReg3" value="등록 완료">
			</div>
		</div>
	</div>
	...
	<section class="section">
		<!-- 복붙영역(html) 시작, 복붙시 바꿔야할 값들 : id, data-ng-controller  -->
		<div id="data-uiGrid-3" data-ng-controller="dataCtrl-3" style="height: auto;">
			<div data-ui-i18n="ko" style="height: 220px;">
				<div data-ui-grid="gridOptions" class="grid" data-ui-grid-edit data-ui-grid-cellNav ui-grid-pagination data-ui-grid-resize-columns data-ui-grid-auto-resize ui-grid-auto-resize data-ui-grid-selection data-ui-grid-exporter ui-grid-pinning>
					<div data-ng-if="loader" class="loader"></div>
					<div class="watermark" data-ng-show="!gridOptions.data.length">데이터가 없습니다.</div>
	...
	<script>
		...
		$(document).ready(function() {
			...
			document.getElementById("split_limit_1").value = "0.5";
			document.getElementById("split_limit_2").value = "15.0";
			document.getElementById("split_limit_3").value = "0.5";
			...
			list_scope.gridApi.selection.on.rowSelectionChanged(list_scope, function(row) {
				...
				data_scope1.reloadGrid({
					master_id : row.entity.MASTER_ID,
					ingredient_id : "YPIG-0001"
				});

				data_scope2.reloadGrid({
					master_id : row.entity.MASTER_ID,
					ingredient_id : "YPIG-0007"
				});

				data_scope3.reloadGrid({
					master_id : row.entity.MASTER_ID,
					ingredient_id : "YPIG-0031"
				});
			});
			...
			// EXEC_RFC : "FI"
			var param = {
				listQuery : "yp_zpp.zpp_ore_seller_compare_list", //list가져오는 마이바티스 쿼리 아이디
				cntQuery : "yp_zpp.zpp_ore_seller_compare_cnt" //list cnt 가져오는 마이바티스 쿼리 아이디
			};
			list_scope.paginationOptions = customExtend(list_scope.paginationOptions, param); //위에 선언한 파라미터 오브젝트를 기존 paginationOject에 합
			//복붙영역(앵귤러 이벤트들 가져오기) 끝

			data_scope1 = angular.element(document.getElementById("data-uiGrid-1")).scope(); //html id를 통해서 controller scope(this) 가져옴
			data_scope1.gridApi.selection.on.rowSelectionChanged(data_scope1, function(row) {
				//	console.log("sel_row : " + row.entity.SELLER_ACTION)
				if(row.entity.SELLER_ACTION == "UMPIRE") row.entity.SELLER_ACTION = "SPLIT";
				else row.entity.SELLER_ACTION = "UMPIRE";
			});
			data_scope1.gridApi.selection.on.rowSelectionChangedBatch(data_scope1, function(rows) { //전체선택시 가져옴
			});
			// pagenation option setting  그리드를 부르기 전에 반드시 선언
			// 테이블 조회는 
			// EXEC_RFC : "FI"
			var param1 = {
				listQuery : "yp_zpp.zpp_ore_seller_compare_dt_list", //list가져오는 마이바티스 쿼리 아이디
				cntQuery : "yp_zpp.zpp_ore_seller_compare_dt_cnt" //list cnt 가져오는 마이바티스 쿼리 아이디
			};
			data_scope1.paginationOptions = customExtend(data_scope1.paginationOptions, param1); //위에 선언한 파라미터 오브젝트를 기존 paginationOject에 합
			//복붙영역(앵귤러 이벤트들 가져오기) 끝

			data_scope2 = angular.element(document.getElementById("data-uiGrid-2")).scope(); //html id를 통해서 controller scope(this) 가져옴
			data_scope2.gridApi.selection.on.rowSelectionChanged(data_scope1, function(row) {
				//	console.log("sel_row : " + row.entity.SELLER_ACTION)
				if(row.entity.SELLER_ACTION == "UMPIRE") row.entity.SELLER_ACTION = "SPLIT";
				else row.entity.SELLER_ACTION = "UMPIRE";
			});
			data_scope2.gridApi.selection.on.rowSelectionChangedBatch(data_scope2, function(rows) { //전체선택시 가져옴
			});
			// pagenation option setting  그리드를 부르기 전에 반드시 선언
			// 테이블 조회는 
			// EXEC_RFC : "FI"
			...
			data_scope2.paginationOptions = customExtend(data_scope2.paginationOptions, param2); //위에 선언한 파라미터 오브젝트를 기존 paginationOject에 합
			//복붙영역(앵귤러 이벤트들 가져오기) 끝

			data_scope3 = angular.element(document.getElementById("data-uiGrid-3")).scope(); //html id를 통해서 controller scope(this) 가져옴
			data_scope3.gridApi.selection.on.rowSelectionChanged(data_scope3, function(row) {
				//	console.log("sel_row : " + row.entity.SELLER_ACTION)
				if(row.entity.SELLER_ACTION == "UMPIRE") row.entity.SELLER_ACTION = "SPLIT";
				else row.entity.SELLER_ACTION = "UMPIRE";
			});
			data_scope3.gridApi.selection.on.rowSelectionChangedBatch(data_scope3, function(rows) { //전체선택시 가져옴
			});
			// pagenation option setting  그리드를 부르기 전에 반드시 선언
			// 테이블 조회는 
			// EXEC_RFC : "FI"
			...
			data_scope3.paginationOptions = customExtend(data_scope3.paginationOptions, param3); //위에 선언한 파라미터 오브젝트를 기존 paginationOject에 합
			//복붙영역(앵귤러 이벤트들 가져오기) 끝
