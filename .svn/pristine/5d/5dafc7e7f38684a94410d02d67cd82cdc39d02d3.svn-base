...
<script type="text/javascript">
	...
	function fnSendData(i){
		$("#COMPANY_ID", opener.document).val($("input[name=L_COMPANY_ID]:eq("+i+")").val());		//보고서코드 
		$("#USER_NAME", opener.document).val($("input[name=L_USER_NAME]:eq("+i+")").val());		//보고서코드 
		self.close();
	}
</script>
</head>
						...
						<c:if test="${fn:length(list) > 0}">
							<c:forEach var="list" items="${list}" varStatus="i">
								<tr onclick="fnSendData(${i.index});" style="cursor: pointer;">
									...
									<td>${list.COMPANY_ID}<input type="hidden" name="L_COMPANY_ID" value="${list.COMPANY_ID}"></td>
									<td>${list.CARD_NO}</td>
									<td>${list.USER_NAME}<input type="hidden" name="L_USER_NAME" value="${list.USER_NAME}"></td>
								</tr>
							</c:forEach>
						</c:if>
						<c:if test="${fn:length(list) == 0}">
							<tr>
								<td align="center" colspan="4">조회된 내역이 없습니다</td>
							</tr>
						</c:if>
					</table>
				</div>
