			...
			<c:choose>
				<c:when test="${fn:length(alllist) > 0}">
					<c:forEach var="list" items="${alllist}" varStatus="i">
						<tr>
							<%-- s:문자, n:숫자, d:날짜 --%>
							<td t="s">${list.COMPANY_ID}</td>
							<td t="s">${list.USER_NAME}</td>
							<td t="s">${list.A_START_DATE}</td>
							<td t="s">${list.A_END_DATE}</td>
							<td t="s">${list.AUFNR}</td>
							<td t="s">${list.KTEXT}</td>
							<td t="s">${list.AUFTEXT}</td>
							<td t="s">${list.W_START_DATE}</td>
							<td t="s">${list.W_END_DATE}</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:when test="${fn:length(alllist) == 0}">
