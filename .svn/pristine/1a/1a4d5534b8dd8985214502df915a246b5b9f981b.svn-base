...
<body>
		...
		<div class="tbl_box">
			<table cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<th>날짜</th>
					<td>
						<input type="text" name="S_START_DATE" id="S_START_DATE" class="default_hide calendar search_dtp_d" value="${today}" readonly="readonly"/>
					</td>
					<th>&nbsp;</th>
					<td>&nbsp;</td>
				</tr>
	...
	<script>
				...
				columnDefs : [ //컬럼 세팅
				{
					...
					aggregationType: function ()
			        {
			            ...
			            for (var i = 0; i < visibleRows.length; i++) {
			              if(visibleRows[i].entity.S_START_DATE !== null) {
			                  sum += parseInt(visibleRows[i].entity.WORKER_CNT);
			              }
			            }
			            return sum;
			        },
					footerCellTemplate: '<div class="ui-grid-cell-contents right" >{{col.getAggregationValue()}}</div>'
				} ]
			});
		...
		$(document).ready(function() {
			...
			// EXEC_RFC : "FI"
			var param = {
				listQuery : "yp_zwc_cmt.select_zwc_cmt_list", //list가져오는 마이바티스 쿼리 아이디
				cntQuery : "yp_zwc_cmt.select_zwc_cmt_list_cnt" //list cnt 가져오는 마이바티스 쿼리 아이디
			};
			...
			$("#search_btn").on("click", function() {
				scope.reloadGrid({
					...
					S_START_DATE  : $("#S_START_DATE").val(),		//날짜(출입일)
