...
<title>정광 심판판정 의뢰 대상 조회 [현재는 사용하지 않음]</title>
</head>
<body>
				...
				<tr>
					...
					<td>
						<select name="ingredient_id">
							<option value="YPIG-0001">Zn</option>
							<option value="YPIG-0007">Ag</option>
							<option value="YPIG-0031">Au</option>
 						</select>
					</td>
					<th>SplitLimit</th>
					<td colspan="1">
						<input type="text" name="split_limit" ><label>g/T</label>
					</td>
				</tr>
			</table>
			<div class="btn_wrap">
		...
		$(document).ready(function() {
			...
			list_scope.gridApi.selection.on.rowSelectionChanged(list_scope, function(row) {
				...
				data_scope.reloadGrid({
					...
					ingredient_id : $("select[name=ingredient_id]").val()
				});
			});
			...
			// EXEC_RFC : "FI"
			var param = {
				listQuery : "yp_zpp.zpp_ore_umpire_query_list", //list가져오는 마이바티스 쿼리 아이디
				cntQuery : "yp_zpp.zpp_ore_umpire_query_cnt" //list cnt 가져오는 마이바티스 쿼리 아이디
			};
			...
			data_scope.gridApi.selection.on.rowSelectionChanged(data_scope, function(row) {
				//	console.log("sel_row : " + row.entity.YP_ACTION)
				if(row.entity.YP_ACTION == "UMPIRE") row.entity.YP_ACTION = "SPLIT";
				else row.entity.YP_ACTION = "UMPIRE";
			});
			...
			// EXEC_RFC : "FI"
			var param2 = {
				listQuery : "yp_zpp.zpp_ore_umpire_query_dt_list", //list가져오는 마이바티스 쿼리 아이디
				cntQuery : "yp_zpp.zpp_ore_umpire_query_dt_cnt" //list cnt 가져오는 마이바티스 쿼리 아이디
			};
