...
<body>
	...
	<table id="tableData" border="1">
		...
		<thead>
			<tr>
				<th rowspan="2">연도</th>
				<th rowspan="2">계약코드</th>
				<th rowspan="2">계약명</th>
				<th colspan="2">계약기간</th>
				<th rowspan="2">거래처</th>
				<th rowspan="2">담당부서</th>
				<th rowspan="2">코스트센터</th>
				<th rowspan="2">도급구분</th>
				<th rowspan="2">단위</th>
				<th colspan="2">소급</th>
				<th rowspan="2">조업계약</th>
			</tr>
			<tr>
				<th>시작</th>
				<th>종료</th>
				...
				<c:when test="${fn:length(alllist) > 0}">
					<c:forEach var="list" items="${alllist}" varStatus="i">
						<tr>
							...
							<td t="s">${list.CONTRACT_CODE}</td>
							<td t="s">${list.CONTRACT_NAME}</td>
							<td t="s">${list.START_YYYYMM}</td>
							<td t="s">${list.END_YYYYMM}</td>
							<td t="s">${list.VENDOR_NAME}</td>
							<td t="s">${list.DEPT_NAME}</td>
							<td t="s">${list.COST_NAME}</td>
							<td t="s">${list.GUBUN_NAME}</td>
							<td t="s">${list.UNIT_NAME}</td>
							<td t="s">${list.RETROACT_START_DATE}</td>
							<td t="s">${list.RETROACT_END_DATE}</td>
							<td t="s">${list.WORKING_GUBUN}</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:when test="${fn:length(alllist) == 0}">
